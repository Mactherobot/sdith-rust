...
\todo{Make sure all code examples are up to date}
...
This modular design ensures that each aspect of the protocol is encapsulated within its respective submodule, making the codebase easier to understand, maintain, and extend. \todo{make sure this is up to date}
\begin{figure}[H]
  \dirtree{%
...
The \rust{[T; N]} array type ensures that the value is stored on the stack. Its possible for us to ensure this due to the nature of \rust{const} (constants) in Rust where the value is known at compile time. However, the digest size would have to change according to the NIST security Categories. For example, for category five we would use SHA-512, we would need to update the size of the digest to 512 bits. This would require us to update the \rust{PARAM_DIGEST_SIZE} constant. There are several ways of solving this problem. The simplest would be to change the hash type to use dynamic heap allocated vectors instead \rust{Vec<T>}. However, this would come at a decrease in performance as data would have to be allocated and fetched. Instead, used the technique described in \autoref{sub:feature_flags}. We introduced the three feature flags \texttt{category\_one}, \texttt{category\_three}, and \texttt{category\_five}. Next we set up the \texttt{crate::constants::params} module. We set up 
...
The SD-in-the-Head protocol is designed in a inherently modular way. This makes it easier to develop its different subroutines step by step. Each subroutine handles a specific \textit{concern}, such as basic Galois Field or cryptographic primitives. By beginning with these smaller subroutines, we could better structure our code, and it also became more straightforward to test, refine, and integrate each component before combining them into the overall system.
...
Our initial implementation focused on the NIST Category one of the specification~\cite{aguilarsyndrome11}, which required the XOF in the form of Shake128~\cite{aguilarsyndrome11}. However our implemementation would have to be able to support different XOF variants to support Category three and five. Furthermore, we wanted to explore the idea that the XOF could be swapped out for a different and more performant implementation -- such as Blake3~\cite{blakethree}. Therefore we designed a trait \rust{SDitHXOF} that allows us to swap the underlying XOF implementation in our \rust{PRG} struct.
...
The generic type parameter \rust{SDitHXOF<T>} allows for the XOF to be swapped out for a different implementation. For example, we could swap out the Shake128 implementation for Blake3~\cite{blakethree} by implementing the trait for \rust{SDitHXOF<Blake3>}. This would allow us to use the same api in the PRG. Next we implemented the base variant with Shake. 
