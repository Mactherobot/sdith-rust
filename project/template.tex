...
However, often it is tedious to keep transferring ownership of values, whenever they need to be used in a function. So how can we avoid this boilerplate of returning ownership? Rust provides a way of avoiding this transfor of ownership by using \textit{references}. References allow you to \textit{borrow} the value without taking ownership of it. This is done by using the \rust{&} symbol. Consider the example
\begin{minted}{rust}
  fn main() {
    let d = String::from("hello");
...
The \textit{lifetime} of an value on the stack is the duration that an value is guaranteed to be valid in the scope. Most often, the developer will not have to ever worry about the lifetime as the compiler will infer it. Lifetimes of a value starts when it is assigned ownership and ends when it gets dropped or the ownership is moved.
\begin{minted}{rust}
  let r: &Item;
  {
...
\end{minted}
While the Rust compiler, most often can infer the lifetime of a value it can sometimes be necessary to explicitly define the lifetime of a value. This is done by using the \textit{lifetime annotation} \rust{'a}. This is most often used when working with references in structs or functions.
\begin{minted}{rust}
...
An example of using the \rust{Result} type is shown below:
\begin{minted}{rust}
