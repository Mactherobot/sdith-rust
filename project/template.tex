...
In this chapter, we provide a specification of the initial SD-in-the-Head protocol, along with the details specific to the implemented threshold variant. We include pseudo-code for the major subroutines and algorithms, as well as a comprehensive description of the parameters used in the protocol. Finally, we present an overview of the associated security guarantees.
...
\begin{quote}
  ...
  \end{align*}
  for $x = (x_a | x_b)$. We have the polynomial representation of the problem
  \begin{align*}
...
To create a public and private key pair, we need a witness for the ZK in the form of a syndrome decoding (SD) instance. 
\begin{align*}
  y = HVx
\end{align*}
...
Next, we can compute the polynomials $S$ and $P$. Recall, that $S$ is the lagrangian interpolation of the $x$ vector -- s.t. $S(i) = x_i$ -- while $P = S \cdot Q / F$.

In a naive implementation, we would have the parties of the MPC protocol each locally compute the interpolation of their share of $x$ to obtain $S$. However, this would be computationally expensive.

Instead, we leverage the \textit{Lagrange interpolation structure} for both $S$ and $P$. $S$ is given by the lagrange basis polynomial $L_i(X)$ as
\begin{align*}
...
We store the precomputations in a temporary array \rust{tmp_poly} and then transfer the result to the polynomial \rust{s_poly} using \rust{gf256_add_vector}.
