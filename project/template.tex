...
The SD-in-the-Head protocol is founded on the computational hardness of the Syndrome Decoding (SD) problem for random linear codes over a finite field, as described in \autoref{sec:syndrome}. To recall, the problem is formally stated as follows:
\begin{quote}
...
In its basic form, the MPC protocol is assumed to be a $(N-1, N)$-private MPC protocol. However, the protocol can be optimized using $(\ell, N)$-private threshold secret sharing, where $\ell < N - 1$. This threshold variant adjustment allows for improved efficiency while maintaining security.

To analyze the security of the protocol, we define its false positive probability, denoted as $p$. The false positive probability quantifies the likelihood that the MPC protocol incorrectly accepts an invalid instance. The following lemma formally characterizes $p$:
...
\subsection{Avoiding interpolations for $S$}\label{sec:sdith-mpc-avoid-interpolation}
...
To obtain a signature scheme from the MPC protocol \autoref{def:sdith-mpc}, two transformations are needed. First, into a ZK-POK using MPCitH and second, into a signature scheme using the Fiat-Shamir transform.

\subsubsection*{ZK-POK from MPCitH}

\autoref{sec:mpcinth}

\begin{protocol}[ZK-POK from MPCitH] The ZK-POK is run as follows
  \begin{itemize}
    \item The prover $\mathcal{P}$ generates input sharing and commites to the party shares.
    \item The verifier $\mathcal{V}$ challenges $\mathcal{P}$ with randomness $r,\epsilon$ of \autoref{def:sdith-mpc}.
    \item $\mathcal{P}$ runs \autoref{def:sdith-mpc} \textit{in their head} and sends the broadcast values to $\mathcal{V}$.
    \item $\mathcal{V}$ challenges $\mathcal{P}$ to open all parties in $I \subsetneq [1 : N]$
    \item $\mathcal{P}$ opens and reveals the commitment values of all parties in $I$.
    \item $\mathcal{V}$ checks the consistency of the MPC protocol values and the commitment values.
  \end{itemize}
\end{protocol}
...
\subsubsection{$GF(q^4)$}\label{sec:gf_} % (fold)
...
\subsubsection{Multiplication in $GF(q^4)$}\label{sec:multiplication_in_gf_q_} % (fold)

% subsubsection Multiplication in $GF(q^4)$ (end)
