...
\paragraph{Merkle Tree Commitment Scheme}
...
\paragraph{Threshold SSS}\label{sub:threshold-sss}
...
\paragraph{Shamir's Secret Sharing}\label{sub:shamir}
...
A majority of constructions within the MPCitH framework~\cite{baum2020concretely,feneuil2022syndrome,katz2018improved} rely on $ (N-1, N) $ additive secret sharing schemes (SSS). Feneuil and Rivain~\cite{feneuil2023threshold,feneuil2023threshold2} explore the application of threshold secret sharing schemes (\nameref{sub:threshold-sss}). In addition to improving the soundness of the resulting zero-knowledge protocol, their approach reduces the computational overhead: the prover needs to emulate only $ \ell + 1 $ parties, while the verifier needs to verify and emulate just $ \ell $ parties. This optimization delivers significantly improved performance, particularly for low-threshold schemes.
...
We define a threshold MPC protocol, $ f_{thr} $, based on a $ (\ell, N) $-private SSS. In the context of the SD-in-the-Head protocol, this can be instantiated with Shamir's Secret Sharing (\nameref{sub:shamir}).
...
To facilitate this, a Merkle Tree Commitment Scheme (\nameref{sub:merkle_tree_prelim}) is employed. This scheme enables the prover to efficiently prove the consistency of a subset of committed inputs by providing additionally the authentication path along with the tree root -- introducing the slight increase in communicational complexity.
...
\begin{lemma}[False positive probability $p$ of \autoref{def:sdith-mpc}]\label{lem:sdith-mpc-soundness}
  ...
  \end{align*} over the randomness $r$ and $\epsilon$ and $\Delta = |\mathbb{F}_{q^4}|$.
\end{lemma}

\paragraph{Threshold Variant Using Shamir's Secret Sharing.}
The threshold variant of the MPC protocol (\autoref{def:sdith-mpc}) can be achieved by incorporating \textit{Shamir's secret sharing}, as described in \nameref{sub:shamir}. By replacing the additive secret sharing scheme with Shamir's polynomial-based scheme, the protocol is extended into a \((\ell, N)\)-threshold MPC protocol, where any \(\ell\) out of \(N\) parties can reconstruct the secret, while fewer than \(\ell\) parties gain no information about the secret.
...
\begin{protocol}[ZK-POK from MPCitH~\cite{feneuil2022syndrome}]\label{def:sdith-zkpok}
  ...
  Both the prover $\mathcal{P}$ and the verifier $\mathcal{V}$ hold public SD instance $(H,y)$. $\mathcal{P}$ holds $x$ such that $y = Hx$ and $\text{wt}(x) \leq w$. We assume a $(\ell, N)$-threshold secret sharing scheme.

  $\mathcal{P}$ computes the proof witness $w = (x, Q, S, P)$ from $x$ and public polynomial $F$.
  ...
  \begin{itemize}[parsep=0pt, itemsep=0pt, topsep=0pt]
    \item $\mathcal{P}$ generates random sharings $\sh{w}, \sh{a^j}, \sh{b^j}, \sh{c^j}$ s.t. $a^j, b^j \leftarrow \psi$ and $c^j = a^j \cdot b^j$ for $j \in [t]$
    \item $\mathcal{P}$ computes the commitments for $i \in [N]$
          \begin{align*}
            \texttt{com}^j_i = \texttt{Commit}(\sh{w}_i, \{ \sh{a^j}_i, \sh{b^j}_i, \sh{c^j}_i \})
          \end{align*}
    \item $\mathcal{P}$ generates $\texttt{root}^j = \texttt{MerkleTree}(\texttt{com}^j_1, \ldots, \texttt{com}^j_N)$ and sends it to $\mathcal{V}$\footnote{see \nameref{sub:merkle_tree_prelim}}
  \end{itemize}

  \paragraph{Round 2}
  $\mathcal{V}$ samples for each $j \in [t]$ random points $r^j \in \mathbb{F}_{q^4}$ and $\epsilon^j \in \mathbb{F}_{q^4}$ and sends them to $\mathcal{P}$.

  \paragraph{Round 3} $\mathcal{P}$ simulates the MPC protocol \textit{in their head} for a subset $S \subseteq [N]$ of $\ell + 1$ parties
  \begin{align*}
    \sh{\alpha^j}_i, \sh{\beta^j}_i, \sh{v^j}_i \leftarrow \texttt{MPC}(\sh{x_A}_i, \sh{Q}_i, \sh{P}_i, \sh{a}_i, \sh{b}_i, r^j, \epsilon^j) \\ \text{for }j \in [t] \text{ and } i \in S
  \end{align*}
  ...
  \paragraph{Round 5}
  The prover opens the parties in $I$ and sends $(\sh{w}_i, \sh{a^j}_i, \sh{b^j}_i, \sh{c^j}_i)_{i \in I}$ to $\mathcal{V}$. $\mathcal{P}$ further sends the \texttt{MerkleTree} authentication paths to $\mathcal{V}$ with
  \begin{align*}
...
\begin{lemma}[Soundness of \autoref{def:sdith-zkpok}]
  Suppose that there exists an efficient prover \(\widetilde{P}\) such that, on input \((H, y)\), it convinces the honest verifier \(V\) on input \(H, y\) to accept with probability:
  \[
    \widetilde{\varepsilon} := \Pr\left[ \langle \widetilde{P}, V \rangle(H, y) \to 1 \right] > \varepsilon,
  \]
  where the soundness error \(\varepsilon\) is given by:
  \[
    \varepsilon = p + \frac{1}{N} - p \cdot \frac{1}{N}.
  \]
  ...
  \[
    \frac{4}{\widetilde{\varepsilon} - \varepsilon} \cdot
    \left( 1 + \widetilde{\varepsilon} \cdot \frac{2 \cdot \ln(2)}{\widetilde{\varepsilon} - \varepsilon} \right).
  \]
\end{lemma}

To further strengthen the protocol against soundness error, \textbf{parallel repetition} is employed. Specifically, the SD-in-the-Head protocol repeats the underlying ZK-POK protocol \(\tau\) times, where:
\[
...
\paragraph{Public-Key Interpretation}
Firstly, note that the public knowledge of an SD instance naturally transforms the protocol into a \textit{public-key signature scheme}. In this setting, the pair \((H, y)\) serves as the public key, while \(x\) functions as the corresponding private key.

To limit the size of the public key, we introduce a seed \( \texttt{sd}_H \) which is used to generate the random matrix \(H\) deterministically via a pseudo-random generator (\(\texttt{PRG}\)). This reduces the storage cost significantly, as only the seed \( \texttt{sd}_H \) needs to be shared, while the full matrix \(H\) can be reconstructed on demand.
...
We assume the existence of two hash functions, \(\texttt{Hash}_1\) and \(\texttt{Hash}_2\), and a pseudo-random generator (\(\texttt{PRG}\)).

The prover generates the challenges as follows:

\begin{enumerate}
  \item \textbf{First Challenge (Randomness for MPC Protocol):}
        Compute the hash \( h_1 \) based on the commitments:
        \begin{align*}
          h_1         & = \texttt{Hash}_1(\texttt{com}_1, \dots, \texttt{com}_N), \\
          r, \epsilon & \leftarrow \texttt{PRG}(h_1).
        \end{align*}

  \item \textbf{Second Challenge (\( i^* \)):}
        Include the message \( m \), along with salt, randomness, and MPC outputs into the second hash:
        \begin{align*}
          h_2 & = \texttt{Hash}_2(m, \texttt{salt}, h_1, \sh{\alpha}, \sh{\beta}, \sh{v}), \\
          i^* & \leftarrow \texttt{PRG}(h_2).
        \end{align*}
\end{enumerate}

\paragraph{Message Inclusion and Pre-Processing}
It is important to note that the message \( m \) is only incorporated into the second hash function \( \texttt{Hash}_2 \). This design enables pre-processing of the first three rounds (\textbf{Round 1--3}) independently of the message, which allows for significant efficiency improvements in practical implementations.

\paragraph{Signature Construction}
The resulting signature consists of the following components:
\begin{align*}
  h_1, h_2, \sh{x_A}_i, \sh{P}_i, \sh{Q}_i, \sh{a}_i, \sh{b}_i, \sh{c}_i \quad \text{for } i \in I, \quad
  \{ \texttt{com}_i, \sh{\alpha}_i, \sh{\beta}_i, \sh{v}_i \}_{i \notin I}.
\end{align*}


\section{Threshold Variant}

Omit $h_2$ from signature.
...
\paragraph{Functions and ownership}\label{sub:rustlifetimes} % (fold)
...
\paragraph{The Type System}\label{sub:rusttypes}
...
\paragraph{Error handling}\label{sub:rusterror} % https://www.lurklurk.org/effective-rust/panic.html
...
\paragraph{Code Documentation}\label{sub:code-documentation}
...
\paragraph{Modules}\label{sub:rust_modules}
...
\paragraph{Visibility} %pub(crate)
...
\paragraph{Hashing}\label{sec:hashing} % (fold)
...
First, as mentioned in the \nameref{sub:constants}, we defined a primitive type for the Hash which we used to define a generic trait:
\begin{minted}{rust}
...
\end{minted}
We then implemented the trait, as was shown in \nameref{sub:prg}, for the following hash primitives SHA3-256, SHA3-384 and SHA3-512.
...
\paragraph{authPath and getAuthSize}\label{sub:auth_path}
...
\paragraph{getRootFromAuthPath}\label{sub:get_root_from_auth_path}
...
\paragraph{First Simple Extension}\label{sub:first_simple_extension}
...
As mentioned before in the \nameref{sub:field_arithmetic} we have a trait for the required field operations, so we can implement those on the type $FPoint$.
...
We test the extended field arithmetic by using the same test as we defined for the trait in \nameref{sub:field_arithmetic}.
...
\paragraph{Testing the Witness Generation}\label{sub:testing_our_witness_generation}
...
We ran benchmarking tests for different areas of the codebase -- based on the api when they were specifically relevant to optimizations from \nameref{sub:rust_optimizations}. Furthermore, we ran benchmarks for different configurations, run through feature flags. We ran all benchmarks for both the baseline and release profiles and ran them on both Setup 1 and Setup 2 \nameref{sub:testing_setup}.
...
\section{MPC computation toy example}
