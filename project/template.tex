...
In this section we will go through the mathematical background needed to understand the basis and individual parts of the SD-in-the-Head scheme. First,  we will describe the Galois finite field $\mathbb{F}_q$ used in the scheme \cite{martinez2023syndromes, reed1960polynomial}. Next, we will describe the Syndrome Decoding problem instance \cite{aguilarsyndrome11,mceliece1978public,berlekamp1978inherent, baldi2013optimization}. Next, we describe the \textit{Multi-Party-Computation-in-the-Head} paradigm and the polynomial reduction of the SD instance that makes it possible to use the paradigm \cite{baum2020concretely} in the schemes. Additionally, we will describe the linear secret sharing scheme that enables the performant \textit{threshold variant}~\cite{feneuil2023threshold}. Finally, we will describe the Fiat-Shamir heuristic that is used to turn the scheme into a signature scheme \cite{fiat1986prove}.
...
% \subsection{This is just notes from the fiat shamir paper}
% The first part is about the identification protocol used for creating smart cards or other ID cards. The protocol uses the fact the factorising is hard for a moduli $n = q \cdot p$, where $p$ and $q$ are large primes. For the identification protocol a trusted center is needed which they suggest to be a government or another type of trusted entity. The protocol is as follows:
% The trusted center will do the following before issuing any IDs:
% \begin{itemize}
%   \item Pick a modulo $n = p \cdot q$
%   \item Pkck a pseudo random function $f$. Mapping from arbitrary strings to $\[0,n)$
% \end{itemize}
% when provided some identification string $I$ on a new user a ID card can be created by the trusted center in the following way:
% \begin{itemize}
%   \item Compute values $v_j = f(I,j)$ for small values of $j$.
%   \item Pick $k$ distinct values of $j$ for which $v_j$ is a quadratic residue modulo $n$. Then compute the smallest square root $s_j$ of $v_j^{-1}\pmod{n}$
%   \item Issue the ID card containing $I$, the $k$ $s_j$ values and their indexes.
% \end{itemize}
%
% Now for verification, a standalone processing unit needs to only contain the moduli $n$, and the function $f$. It can then by doing a simple flow of sending a challenge verify the identity of A. By some simple mathematical properties. They then go through several proofs that ensures that the verifier only accepts the identity of A if the math holds. And that the security is $2^{-kt}$ where $k$ is the number of distinct values, and $t$ is the challenge rounds in the verifying protocol.
%
% Furthermore, it defines a lemma that this protocol is, for a fixed length $k$ and arbitrary  $t$, a zero-knowledge proof. Where A can show to B that A is actually A without revealing any information about A to B.
%
%
%
% \todo{Turning the ZKAoK into a signature scheme}
%
% Such a zero-knowledge protocol can then be turned into a signature scheme by relying on the pseudo random function $f$ instead of having B send a challenge. 
% So in order to sign a message $m$, the prover A will do the following:
% \begin{itemize}
%   \item Pick random values
%   \item Compute the challenge locally by using the pseudo random function $f$ with the message. 
%   \item Now use the challenge to prepare a value which prooves that A is actually A, and send the identification information, message, challenge and the prepared value that prooves the identity of A to B
% \end{itemize}
% Now for verifying A's signature on $m$ B does the following:
% \begin{itemize}
%   \item Compute the corresponding values based on the function and the identification information. Now verify that B can compute the same challenge values and only accept if they match.
% \end{itemize}
% The security relies on the fact that the moduli $n$ is sufficiently large and the pseudo random function is truly random.
