...
For our implementation we chose the \textit{threshold} variant of the SD-in-the-Head protocol~\cite{aguilarsyndrome11,feneuil2023threshold} as this variant introduces the biggest performance increases, when compared to the initial protocol~\cite{feneuil2022syndrome} and the \textit{hypercube} variant~\cite{aguilarsyndrome11, aguilar2023return} at the cost of a slightly larger signature size.

In this chapter, we will provide a specification of the initial SD-in-the-Head protocol along with the specifics of the threshold variant. We will include pseudo-code for major subroutines and algorithms, as well as a detailed description of the parameters used in the protocol.
