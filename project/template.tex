...
%TODO: Move the first maybe to the introduction
...
%TODO: Rewrite this when the order is set in stone
We begin by introducing the essential cryptographic primitives that underpin the protocol. These include \textbf{collision-resistant hash functions}, the \textbf{Merkle tree commitment scheme} \cite{becker2008merkle}, and \textbf{Galois Finite Fields} $\mathbb{F}_q$ \cite{martinez2023syndromes, reed1960polynomial, brownadvanced}. In addition, we provide short introductions to the foundational protocols of \textbf{Secure Multi-Party Computation} (MPC) and \textbf{Zero-Knowledge} (ZK) proofs, which serve as the underlying cryptographic protocols.
...
Building upon these foundations, we lay the groundwork for the construction of the SD-in-the-Head protocol. First, we discuss the \textbf{Multi-Party-Computation-in-the-Head} (MPCitH) paradigm, explaining how it is leveraged to construct efficient ZK proofs \cite{ishai2007zero}. Next, we introduce the \textbf{Fiat-Shamir heuristic}, a powerful tool that transforms an interactive ZK protocol into a non-interactive signature scheme \cite{fiat1986prove}. Additionally we detail the role of the MPC preprocessing \cite{baum2020concretely} and linear secret-sharing scheme \cite{feneuil2023threshold}, which dramatically improves the efficiency of the MPCitH construction, and verification.

\section{Cryptographic Primitives}
%TODO: maybe move some of the information from the section above to here.
% and maybe rename 
...
%TODO: is the r(x) correct. In ours we represent it with z, which is not the x value that you times 32.
\begin{align*}
...
\begin{itemize}
  ...
  \item \textbf{Hiding}: The value is hidden from the verifier until the prover reveals it.
\end{itemize}
...
\begin{definition}
  \label{def:mpcinth_properties}
  The basic MPC-in-the-Head protocol has the following properties:
  \begin{itemize}
    \item \textbf{Completeness}: A boolean function is said to be complete if it depends on all inputs of the function. Given an honest prover, $\mathcal{R}(x,w) = 1$ and the correctness of $\Pi_f$, all outputs of $P_i$ are one and all views are consistent.
    \item \textbf{Soundness}: If the statement $x$ is false, i.e. $x \notin L$, then $\mathcal{R}(x,w) = 0$ for all $w$. By the correctness of $\Pi_f$, the output of all parties must be $0$. If the verifier accepts, then the prover must have created $\ell$ inconsistent views. This happens with probability at most $p = 1 / \binom{n}{\ell}$. The error probability can be reduced to $2^{-k}$ by repeating the protocol $O(kn^2)$ times.
    \item \textbf{Zero-Knowledge}: The verifier only sees $\ell$ views, and therefore from the definition of $\Pi_f$ learns nothing of the secret witness $w$.
  \end{itemize}
\end{definition}

The basic MPC-in-the-Head protocol can be used to construct a ZK proof for any NP relatio and has been shown to produce fairly efficient ZK protocols. Feneuil
...
\begin{enumerate}
  \item The parties generate a random $\epsilon \in \mathbb{F}$, for a finite field $\mathbb{F}$.
