...
For the $GF(2^4)$ field, we did not implement the functions as a trait, as they are solely used as sub-functions for the $GF(2^8)$ field. To represent an element in the $GF(2^4)$ field, we used a 2-byte array, representing $(a, b) = a + bX$. 
...
\paragraph{Testing the $GF(2^4)$ Field}

Since the $GF(2^4)$ field is only used as a sub-function for the $GF(2^8)$ field, we implemented a minimal test to verify correctness. Specifically, we checked that a simple multiplication by 32 is consistent with the operation $(a, b) \cdot (0, 32)$:
...
More extensive testing was reserved for the $GF(2^8)$ field, as it is the primary target of our implementation.
...
Now that we have the $GF(2^4)$ field we can implement the $GF(2^8)$ field arithmetic. We represent an element in the $GF(2^8)$ field as a 4-byte array. So we have a representation of $(a,b,c,d) = a + bX + cX^2 + dX^3$. 
