...
The authors, Aguilar et al, of the SDitH protocol supply a detailed and well-crafted specification, along with a reference implementation in C, which serves as a robust starting point for our work.
...
\begin{itemize}
  ...
  \item To utilise the built-in testing framework of Rust to ensure the correctness of our implementation. We aim to provide a comprehensive test suite that covers the mathematical properties of the protocol and integration tests to verify the correctness of the implementation.
...
\begin{definition}
  ...
  \begin{itemize}
    \item \textbf{One-way}: It is computationally infeasible to invert the function on any output for any unknown input.
...
\begin{definition}[ROM Paradigm]
  Assume one has a protocol problem $\Pi$, this could for example be that of public key encryption or signing of a message, where the problem is independent from the choice of $h$. In order to devise a good protocol $P$ for $\Pi$:
  \begin{itemize}
    ...
    \item Replace the random oracle with a hash function $h$
  \end{itemize}
\end{definition}

We formally define the \textit{classical} ROM as follows:
\begin{definition}[Random Oracle Model (ROM)]
  The adversary is given access to an oracle $O: \{0,1\}^* \rightarrow \{0,1\}^*$, that acts as a completely random hash function. The adversary can query the oracle $O(x)$ and receive the output. The oracle is stateless and the same input will always yield the same output.
\end{definition}
This can also be thought of as a black box, that on any input will give a completely random output.
...
In an effort to improve this, Baum and Nof\cite{baum2020concretely} introduced a variant of the framework based on a specialized MPC protocol. This protocol efficiently verifies a product relation $a \cdot b = c$. Their contributions significantly enhanced the efficiency of MPCitH-based ZK-PoK protocols and serve as the foundation for the underlying MPC protocol used in the SDitH protocol.
...
\begin{protocol}[MPC Multiplication using Beaver triples]\label{def:Beaver-multiplication}
  Parties seek to compute the product of two shares $\sh{x}, \sh{y}$ secretly, given a $+$-homomorphic additive secret sharing scheme and a preprocessed random Beaver triple $\sh{a}, \sh{b}, \sh{c}$. The parties do the following:
...
Within the context of the SDitH protocol, the transformation from a ZK-PoK to a signature scheme is accomplished by substituting the verifier's \textit{MPC challenge} and \textit{view opening challenge} with the output of a hash function, where the hash function takes as input the prover's prior communications.

\chapter{The Rust Programming language}\label{sec:rust}
...
\section{Cargo - Rust package manager}\label{sec:cargo}
...
\section{Memory safety}\label{sec:rustborrow} % mut references
...
\section{Correctness (Types, Testing and Error handling)}
...
\subsection{The Type System}\label{sub:rusttypes}
...
\subsection{Error handling}\label{sub:rusterror}
...
\section{Maintainable and readable code}
...
\subsection{Code Documentation}\label{sub:code-documentation}
...
\subsection{Codebase structure and Modules}\label{sub:rust_modules}
...
\subsection{Conclusions on Maintainability and Readability}
...
\section{Dynamic compilation and Feature flags}\label{prelim:feature_flags}
...
A good use case could be facilitating the NIST security parameters. Note that we want the parameters to be constant at compile time, but we do not want to create several instances of functions that use each parameter for each category. Rust's \textit{feature flags} come in handy.
...
\section{Optimizations in Rust}\label{sub:rust_optimizations}
...
\begin{table}[]
  \begin{tabular}{p{0.12\textwidth}p{0.78\textwidth}}
    ...
    $a, b, c$                    & Beaver triple satisfying $a \cdot b = c$.                                                              \\
    $\alpha, \beta, v$           & Broadcast values in $\mathbb{F}_{\text{points}}$.                                                      \\
...
\end{align*}
since $Q\cdot S - P \cdot F$ is a polynomial of degree at most $m + w -1$. This is following \autoref{lem:schwartz}. Whenever this event occurs, the probability of a false positive is bounded by
\begin{align*}
...
To accommodate the repetitions, the authors incorporate a slight modification inspired by the Limbo proof system~\cite{delpech2021limbo}. This allows the protocol to reuse the same MPC challenges $r$ and $\varepsilon$, as well as the Beaver triples, across all $\tau$ repetitions. This tweak allows us to modify the MPC computation input so that \textit{plain broadcast} value only needs to be computed once.
...
\begin{protocol}[SDitH Signing Protocol]\label{def:sdith-sign}
  ...
  \begin{enumerate}[itemsep=0pt, topsep=0pt, parsep=0pt]
    \item Compute plain broadcast $\texttt{broad\_plain} \stackrel{\varphi^1_1}{\longleftarrow} \texttt{input\_plain}, r, \varepsilon$.
  ...
  \begin{enumerate}[itemsep=0pt, topsep=0pt, parsep=0pt]
    \item Compute authentication paths $\texttt{auth}^e = \texttt{MerkleAuthPath}(\{\texttt{com}^e_i\}_{i \in I})$.
    \item Compute witness shares $\sh{\texttt{wit}}_i^e = \text{TruncateBeaver}(\texttt{input}^e_i)$.
...
With the potential for new undiscovered quantum algorithms and unknowns in the capacities of the future quantum computers, there remain large uncertainties in estimating the security of the algorithms. In order to deal with these uncertainties, NIST proposed for the 2022 stadardization effort, to define the security of submissions in a range of five categories. Each, with an easy-to-analyze cryptographic primitive providing the lower bound for a variety of metrics deemed relevant to practical security.
...
The complexity and capability of quantum algorithms are measured in terms of quantum circuit size, i.e. the number of quantum gates in the quantum circuit. In order to estimate the quantum security of the signature protocols, circuit size can be compared to the resources required to break the security of \autoref{def:nistsec}.
...
The general syndrome decoding problem \autoref{def:syndrome} is known to be \textbf{NP}-hard. The most well performing attacks are of the family \textit{Information-Set Decoding} (ISD)~\cite{aguilarsyndrome11,prange1962use}. An information-set decoding algorithm iteratively selects a set $I$ of $k$ indices (the information set) and defines $J = [1, \ldots, m] \setminus I$. A step is successful if the submatrix $H_J$ is invertible and the support of $x$ lies entirely in $J$, allowing $x$ to be computed as $x_J = H_J^{-1} y$, with other entries set to $0$ (the number denoted by $\delta$). Successive work by~\cite{stern1989method,peters2009information} gives the following bounds for the complexity of an ISD algorithm. The cost of each iteration is
\begin{align}
  \begin{split}
    & C_{iter} =                                                                                  \\
    & \frac{1}{2}(n-k)^2(n+k) + \left(\left(\frac{k}{2} - \rho + 1\right) +
    \left(\binom{\lfloor k/2 \rfloor}{\rho} + \binom{k - \lfloor k / 2 \rfloor}{\rho}\right)(q-1)^\rho\right) \delta \\
    & + \frac{q}{q-1}(w-2\rho+1) 2\rho \left(1 + \frac{q-2}{q-1}\right)
    \frac{\binom{\lfloor k/2 \rfloor}{\rho} + \binom{k - \lfloor k / 2 \rfloor}{\rho}(q-1)^{2\rho}}{q^\delta},
  \end{split}
\end{align}

Where parameters $\rho$ defines a number of positions chosen in the the information subsets $X,Y$. The parameter $\rho$ is chosen quite small to minimize the cost of the algorithm and $\delta$ is set to \[ \delta = \log_q\binom{k/2}{\rho} + \rho \log_q(q-1) \] The success probability for each iteration is

\begin{align}
  p_{succ} \binom{\left\lfloor k/2 \right\rfloor }{\rho} \binom{k -\left\lfloor k/2 \right\rfloor }{\rho} \binom{n-k-\delta}{w-2\rho} / \binom{n}{w},
\end{align}
...
As mentioned in the \nameref{def:sdith-sign}, we need a collision-resistant hash function. For this we also used the \texttt{tiny-keccak} crate. And again due to the different security categories, we had to be able to switch between different implementations of the underlying hash function.
...
To maintain full control over the field arithmetic and avoid dependencies on external libraries, we implemented the Galois field operations internally. This approach enabled us to fine-tune the implementation
...
The trait implementation enables the use of field arithmetic in our codebase and makes it so that we will not have to change any of the utilising code if we were to switch to a different field -- for example the GF251 field proposed by \cite{aguilarsyndrome11}. As an example, the computation of the inner product for Beaver triples can be expressed as follows -- regardless of the field we choose to implement.
\begin{minted}{rust}
...
\begin{enumerate}
  ...
  \item \textit{Shift-and-Add with Modular Reduction}: This technique -- the one chosen in the reference implementation~\cite{aguilarsyndrome11} -- in particular, provides robust performance in most applications. Importantly, it has no conditional branches or random access table lookups, making it resistant to \textit{timing attacks} -- denoted as \textit{constant-time}.
\end{enumerate}
...
\begin{itemize}[parsep=0pt, itemsep=3pt]
  \item \textit{Combined benchmarks for the Rust implementation} -- Full benchmarks for our implementation and each optimisation flag. We ran benchmarks for our implementation on both setups 1 and 2 \autoref{fig:benchmarking_setups}. We ran all combinations of feature flags for parallelisation, SIMD on the \textit{release} profile. Furthermore, we ran with and without Turbo Boost enabled for \textit{Linux setup}.
  \item \textit{Optimized reference implementation benchmarks} -- For comparison, we benchmarked the reference implementation on \textit{Linux setup} \autoref{fig:benchmarking_setups}. We used \texttt{clang@15} for compilation, which is the same used by~\cite{aguilarsyndrome11}. We ran both with and without Turbo Boost enabled.
...
Benchmarks were conducted on two machines: a Dell XPS 15 9510 with \textit{Ubuntu 22.04.5 LTS} and a MacBook Pro with an M2 Pro chip. These machines represent different architectures, allowing us to observe subtle variations in runtime performance based on hardware differences. Each machine is described in \autoref{fig:benchmarking_setups} as \textit{Linux setup} and \textit{Mac setup} respectively.

\begin{figure}[ht]
  \centering
  \fbox{
    \begin{minipage}[t][6cm][t]{.45\textwidth} % Set a fixed height for the boxes
      \textbf{Linux setup}: Dell XPS 15 9510
  ...
  \fbox{
    \begin{minipage}[t][6cm][t]{.45\textwidth} % Set the same height
      \textbf{Mac setup}: MacBook Pro (M2 Pro)
      \begin{itemize}[label={}, leftmargin=0pt, itemsep=2.5pt, parsep=0pt]
...
\begin{table}[H]
  ...
  \begin{tabular}{l|l|lll}
    ...
    \hline
  \end{tabular}
  \caption{API benchmarks over each category and optimisation feature flag for \textit{Mac setup}. We ran reference implementation benchmarks as the optimised solution was not compatible with \textit{Mac setup}.}
...
\begin{table}[H]
  ...
  \begin{tabular}{l|l|lll}
    ...
    \hline
  \end{tabular}
  \caption{API benchmarks over each category and optimisation feature flag for \textit{Linux setup}. We ran the optimised reference implementation \cite{aguilar2023return} with and without Turbo Boost enabled.}
...
For \textit{Linux setup}, we performed benchmarks with and without Turbo Boost (TB) enabled. In contrast, for \textit{Mac setup}, we could only evaluate the ``reference'' implementation due to architectural incompatibilities that prevented running the optimised implementation with SIMD AVX2 optimisations.

The results for \textit{Linux setup} and \textit{Mac setup} are seen in \autoref{tab:api_dell_results} and \autoref{tab:api_mac_results}, respectively.
...
\begin{table}[H]
  ...
  \begin{tabular}{l|l|rr|rr}
    ...
    \hline
  \end{tabular}
  \caption{Percentage differences between the ``base'' and each of the ``parallel'' and ``SIMD'' optimisations for key generation, signing, and verification across categories for \textit{Linux setup} (S1) and \textit{Mac setup} (S2).}
...
\begin{table}[H]
  ...
  \begin{tabular}{l|lll}
    ...
    \hline
  \end{tabular}
  \caption{Benchmarks for the \textit{nightly} and \textit{stable} channels for Rust on \textit{Mac setup}.}
...
\begin{table}[H]
  ...
  \begin{tabular}{|l|c|r|r|}
    ...
    \texttt{Lookup}        & \ref{dudect_test_1} & -0.1527 (25.7867)       & 226.1397     \\
    \texttt{Lookup}        & \ref{dudect_test_2} & -0.3377 (20.4296)       & 196.4218     \\
    \texttt{Lookup}        & \ref{dudect_test_3} & -0.0186 (25.7284)       & 240.2633     \\
    \texttt{Shift-and-Add} & \ref{dudect_test_1} & 0.1467 (1.5569)         & 3.3149       \\
    \texttt{Shift-and-Add} & \ref{dudect_test_2} & -0.1198 (1.4995)        & 3.3911       \\
    \texttt{Shift-and-Add} & \ref{dudect_test_3} & 0.0056 (1.5284)         & 3.4232       \\\hline
  \end{tabular}
  \caption{Running the \texttt{dudect} tool 1000 times on \textit{Linux setup} and collected averages and standard deviations for the three tests and the two implementations of multiplication. A $t < 5$ indicates that there is no significant difference between the two hypotheses. A $t > 5$ indicates that the difference is significant.}
...
Given these findings, we have set the \texttt{Lookup} implementation as the default and provided \texttt{Shift-and-Add} as an alternative via the feature flag \texttt{mul\_shift\_and\_add}. This decision is driven by the substantial performance differences and the observation that key generation is the sole operation where non-random data is multiplied frequently enough to have an impact. Additionally, in practical scenarios, key generation is typically performed only once and is rarely exposed to timing side-channel attacks, minimizing the necessity to enforce constant-time behavior.
...
We performed a comparison between the defined \texttt{SHA3} and \texttt{SHAKE} hashing algorithms from \cite{aguilarsyndrome11} and the \texttt{Blake3} implementation. Benchmarks were executed with full optimizations and \textbf{Turbo Boost} enabled. The results, shown in \autoref{tab:blake3}, indicate that the \texttt{Blake3} implementation is significantly faster, highlighting the hash functionality as a bottleneck in the protocol implementation. For future practical implementations, it is evident that the development of efficient hashing mechanisms for the SDitH protocol should be prioritized.
...
Our report includes a detailed overview of the underlying theory and protocols, along with formal definitions provided in \autoref{def:sdith-sign} and \autoref{def:sdith-verify}. We ensured that our implementation adhered to consistent terminology and closely followed the protocol descriptions, promoting both accuracy and clarity throughout the project.
...
To obtain a signature scheme from \autoref{def:sdith-mpc}, two transformations are needed. First, we will transform the MPC protocol into a zero-knowledge proof of knowledge (ZK-PoK) using MPCitH. The ZK-PoK described in \autoref{def:sdith-zkpok} is based on the verification by sacrificing triples strategy for MPCitH~\cite{baum2020concretely} detailed in \autoref{sec:mpc_sacrificing} but has key changes to accommodate the threshold variant of the protocol~\cite{feneuil2023threshold,feneuil2023threshold2}.
