...
After defining the trait and general setup for the $GF(2)$ binary arithmetic, we could then implementing the methods for the $GF(2^8)$ field. The field is defined by the polynomial $x^8 + x^4 + x^3 + x + 1$. We implemented the field arithmetic using the irreducible polynomial $x^8 + x^4 + x^3 + x + 1$.

\todo{Write about the implementation of the field extensions}
