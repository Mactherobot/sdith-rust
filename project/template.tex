...
The \textbf{stack} stores values in a last-in-first-out (LIFO) order, meaning the last value added is the first to be removed. You can think of it like a stack of plates: the last plate placed on the stack is the first one to be taken off. All data stored on the stack must have a fixed, known size at compile time.

The \textbf{heap}, in contrast, is more flexible. When you allocate memory for a value on the heap, you request a specific amount of memory, and the heap manager finds a suitable location for it. It then returns a pointer to the allocated memory. A helpful analogy for the heap is dining at a restaurant: you tell the waiter the size of your party, and they find a table that fits your group, noting which table you've been assigned.
...
\subsubsection{Borrowing \& References}
...
\subsection{PRG}
First we implemented a pseudo random generator (PRG) based on a extendable output hash function (XOF). We used a library called \textit{tiny\-keccak}~\cite{tinykeccak}, which contains implementations of Keccak derived functions specified in the NIST FIPS 202 standard, SP800-185 and KangarooTwelve.
...
\section{Package publication}

\subsection{CLI}

\begin{minted}{bash}
  $ sdith keygen --output .
  Generating SDitH key pair.
  Seed: buuDCI/Bi7/ME+/LInazUA==
  Public key saved to "./sdith.pub"
  Secret key saved to "./sdith"
\end{minted}

\begin{minted}{bash}
  $ sdith sign --sk ./sdith --message ./way_of_the_sith_doctrine.txt
  Signing message.
  Reading secret key from file: ./sdith
  Seed: U6ICDuJIQp3/lOM5s/B0Yg==
  Salt: lT48FshXu05R1L9IAOgv3J1nLO4qkbNTnSeJKMM2XoU=

  GCgAAGhlbGxvIHdvcmxklT48FshXu05R1L9IAOgv3J1nLO4qkbNTnSeJKMM2...
\end{minted}

\begin{minted}{bash}
  $ sdith verify --pk sdith.pub --signature signed_sith_doctrine.txt
  Verifying message.
  Reading Public Key from file: sdith.pub
  Reading Signature from file: signed_sith_doctrine.txt
