...
\begin{definition}[Opening a shared value]\label{def:ss-opening}
  Denote \texttt{open}$(\sh{s})$ to be the opening of the shared value $s$ where each party $P_i$ broadcasts its share $s_i$ allowing all participants to reconstruct the secret value $s$.
\end{definition}
...
\subsubsection{MPC over arithmetic circuits}
...
Consider the protocol for a single multiplication gate below by~\cite{baum2020concretely}, based on the MPC protocol described in~\cite{damgaard2012multiparty}. Note, that a similar technique can be applied to square relations. Howeverwe focus here solely on multiplication, as it suffices for the SD relation.
...
This allows us to implement the field arithmetic different types, such as u8 and a point in the extended field:
\inputminted[firstline=81, lastline=115]{rust}{../sdith/src/arith/gf256/gf256_ext.rs}
...
\inputminted[firstline=2, lastline=12]{rust}{../sdith/src/arith/gf256/gf256_arith.rs}
