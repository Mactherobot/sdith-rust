...
The primary modification lies in Step 3, where the verifier $\mathcal{V}$ selects a random subset $E \subset [M]$ to verify the correctness of the triples. In contrast, the sacrificing protocol employs random $\epsilon$ values to verify the correctness. This adjustment allows the verifier to check the correctness of the triples independently, without relying on the prover to generate valid triples.
