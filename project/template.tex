...
\begin{theorem}[Linear functions of \autoref{def:sdith-mpc}]\label{thm:sdith-mpc-linear}
  ...
  \end{align}
  such that each party locally computes $\varphi^1$ and $\varphi^2$ on the input shares $\sh{s_A}_i, \sh{P}_i, \sh{Q}_i, \sh{a}_i, \sh{b}_i, \sh{c}_i$, and randomnes $r, \varepsilon$ producing $\sh{\alpha}_i, \sh{\beta}_i, \sh{v}_i$ as valid sharings of $\alpha, \beta, v$.
\end{theorem}

We tweak the MPC input generation in the following way: Define the \texttt{input\_plain} as the tuple containing the MPC inputs:
\begin{align*}
  \texttt{input\_plain} = (s_A, P, Q, a, b, c)
\end{align*}

Next, we compute $\ell$ uniformly random coefficients $\texttt{input\_coef} \in \mathbb{F}_q^{|\texttt{input\_plain}|}$ and compute the sharing with \textit{Share}:
\begin{align}
  \sh{\texttt{input\_plain}}_i & = \text{Share}(i, \texttt{input\_plain}, \texttt{input\_coef}_1, \dots, \texttt{input\_coef}_\ell) \label{eq:sdith-mpc-share} \\ \nonumber
                               & = \texttt{input\_plain} + \sum_{j=1}^\ell i^j \cdot \texttt{input\_coef}_j
\end{align}
...
\begin{protocol}[SD-in-the-Head ZK-POK protocol]
  ...
  \begin{enumerate}[parsep=0pt, itemsep=0pt, topsep=0pt]
    ...
    \item The verifier challenges the prover with the randomness \(r, \varepsilon\) for the MPC protocol.
    \item The prover executes the MPC protocol (in their head) and sends the broadcast values to the verifier in $h_2$
    \item The verifier challenges the prover to open all the parties in \(I \subset [N]\) for $|I| = \ell$.
...
Zero-knowledge of the protocol is held by the fact that the verifier only receives $\ell$ shares to a $\ell + 1$-private SSS. The soundness of the ZK-POK protocol depends on the binding property of the commitment scheme and the inability of the prover to simulate multiple inconsistent views of the MPC protocol. By employing hash consistency checks and commitment verification, the ZK-POK protocol ensures that a cheating prover cannot deviate significantly from honest execution.
...
\begin{lemma}[Soundness of \autoref{def:sdith-zkpok}]
  ...
  \end{align*}
  where $p$ denotes the false positive probability of the MPC protocol, $N$ is the total number of parties, and $\ell$ is the threshold parameter.
\end{lemma}

The threshold approach slightly degrades the soundness of the protocol. To mitigate this, \textit{parallel repetition} is employed to strengthen the soundness signature protocol. Specifically, the SD-in-the-Head protocol repeats the underlying ZK-POK protocol $\tau$ times, where:
\begin{align*}
  \tau = \left\lceil \log_2\left(\frac{1}{\epsilon}\right) \cdot \lambda \right\rceil,
\end{align*}
with $\lambda$ as the security parameter and $2^{-\lambda}$ as the target soundness error. This parallel repetition reduces the overall soundness error exponentially with the number of repetitions, achieving a negligible error probability in practice.

To accommodate the repetitions, the authors~\cite{aguilarsyndrome11} incorporate a slight modification inspired by the Limbo proof system~\cite{delpech2021limbo}. Specifically, we reuse the same MPC challenges $r$ and $\varepsilon$, as well as the Beaver triples, across all $\tau$ repetitions. This tweak allows us to modify the MPC computation input so that $\textit{broad}$ only needs to be computed once.
...
Finally, we introduce a $\texttt{salt} \in \{0,1\}^{2\lambda}$, which serves as pseudo-randomness for commitments and seed generation. The use of $\texttt{salt}$ ensures that the protocol remains secure and avoids reliance on externally provided randomness.

\subsubsection{Optimizing Signature Size and Verification}
In the original SD-in-the-Head protocol~\cite[Figure 1]{aguilarsyndrome11} -- without threshold SSS -- the signature includes both hashes $h_1, h_2$ and all input shares in $I$. To verify the signature, the verifier recomputes the commitments for each share in $I$, and along with the extra commitment $\texttt{com}_i$ for $i \notin I$, reconstructs $h_1$. The verifier then executes the MPC protocol for the opened shares and recomputes $h_2$ from the resulting output shares.
...
\begin{align}
  \text{TruncateBeaver} : \sh{\texttt{input}}_i = (\sh{s_A}_i, \sh{P}_i, \sh{Q}_i, \sh{a}_i, \sh{b}_i, \sh{c}_i) \mapsto (\sh{s_A}_i, \sh{P}_i, \sh{Q}_i) = \sh{\texttt{wit}}_i
\end{align}
...
\begin{protocol}[SD-in-the-Head Signing Protocol]\label{def:sdith-sign}
  ...
  Given a public SD instance $pk = (H, y)$, secret $sk = (s_A, Q, P)$, message $m \in \{0,1\}^*$ and security parameters $\lambda, \tau$

  \paragraph{Assumptions} We assume the following

  \begin{itemize}[itemsep=0pt, topsep=0pt, parsep=0pt]
    \item a $(\ell + 1, N)$-threshold MPC protocol $\varphi^1, \varphi^2$ (\autoref{def:sdith-mpc}, \autoref{sec:mpc}) with a sharing function \texttt{Share}
    \item A Merkle Tree Commitment Scheme: \texttt{MerkleTree} and \texttt{MerkleAuthPath} (\autoref{sub:merkle_tree_prelim})
    \item secure hash functions $\texttt{Hash}_n$ (\autoref{eq:fiatshamirhash})
    \item and random oracle $\texttt{PRG}$ (\autoref{sec:prelim_hash})
    \item a extendable output hash function \texttt{XOF} (\autoref{sec:xof})
  \end{itemize}
            ...
            \item compute input shares for $i \in [N]$
                  \begin{align*}
                    ...
                    \begin{cases}
                      ...
                      \texttt{Share}(  i, \texttt{input\_plain},                                                    \\
                      \quad \quad \quad \texttt{input\_coef}_1^e, \dots, \texttt{input\_coef}_\ell^e) & \quad i > 1
                    \end{cases}
                  \end{align*}
  ...
  \paragraph{Phase 2} Compute first challenge
  \begin{enumerate}[itemsep=0pt, topsep=0pt, parsep=0pt]
  ...
  \paragraph{Phase 4} Compute second challenge
  \begin{enumerate}[itemsep=0pt, topsep=0pt, parsep=0pt]
...
\begin{protocol}[SD-in-the-Head Verification Protocol]\label{def:sdith-verify}
  ...
  \paragraph{Assumptions} We assume the following
  \begin{enumerate}[itemsep=0pt, topsep=0pt, parsep=0pt]
    \item a $(\ell + 1, N)$-threshold MPC protocol with reverse computation $\phi^1, \phi^2$ (\autoref{def:sdith-mpc}, \autoref{thm:mpc_reverse}, \autoref{sec:mpc}) and sharing function \texttt{Share}
    \item A Merkle Tree Commitment Scheme: \texttt{MerkleRootFromAuthPath} (\autoref{sub:merkle_tree_prelim})
    \item secure hash functions $\texttt{Hash}_n$ (\autoref{eq:fiatshamirhash})
    \item and random oracle $\texttt{PRG}$ (\autoref{sec:prelim_hash})
    \item a extendable output hash function \texttt{XOF} (\autoref{sec:xof})
  \end{enumerate}
    ...
    \item Compute broadcast shares
          \begin{align*}
            ...
            \begin{cases}
              ...
              \texttt{Share}(i, \texttt{broad\_plain},                                                      \\
              \quad \quad \quad \texttt{broad\_coef}^e_1, \dots, \texttt{broad\_coef}^e_\ell) & \quad i > 1
            \end{cases}
          \end{align*}
...
Due to the modularity of the SD-in-the-Head protocol, we were able to set about implementing the initial parts of the protocol as a set of subroutines. In our initial implementation we focused on creating the subroutines based on just the specification. This however proved to be a challenge as some of the definitions were not clear on how to be implemented. So we instead took inspiration in how the reference implementation was structured.
...
\begin{itemize}
  \item \rust{new}: Builds a Merkle tree from a commitment array and salt.
  \item \rust{authPath}: Generates an authentication path for specific leaf indexes.
  \item \rust{getAuthSize}: Determines the size of the authentication path for serialization and signature parsing.
  \item \rust{getRootFromAuthPath}: Computes the root hash from commitments and an authentication path.
\end{itemize}
...
pub fn gf256_monic_polynomial_division(
    ...
    // Monic polynomial: a polynomial whose leading coefficient is 1; e.g. X^3 + 23X^2 + 34X + 45
...
\begin{itemize}
  \item \rust{input\_share\_plain}: The input share of the party
  \item \rust{chal}: The challenge values $(r, \epsilon)$
  \item \rust{h\_prime}: The matrix $H'$
  \item \rust{y}: The syndrome decoding instance $y$
  \item \rust{broadcast}: The broadcast values
  \item \rust{with\_offset}: A boolean to indicate if the polynomial $S$ should be computed as $$s = (s_A|y+H's_A)$$ or $$s = (H's_A)$$ and if the polynomial Q should be $$Q = (Q|0)$$ or $$Q = (Q|1)$$. This is due to the fact that for multiplication in a MPC protocol it is only 1 party that needs to add the last part.
  \item \rust{compute\_v}: A boolean to indicate if the value $v$ in the MPC simulation should be computed, used to avoid unnecessary computation in the plain broadcast computation as $v = 0$.
\end{itemize}
...
To verify and debug our code, we configured our implementation to align with the reference implementation provided in C. This setup required us to execute the reference code to obtain intermediate data, which we then used to compare internal states between the two implementations.

During this process, we identified a bug in the calculation of the view-opening challenges. Specifically, the reference implementation neglected to finalize the SHAKE function before squeezing, which posed an issue because the Tiny Keccak Rust library automatically performs this finalization when initializing a hash or XOF.
