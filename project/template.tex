...
\subsubsection{Threshold variant MPC protocol}\label{sub:sdith-threshold-sss}
...
To accommodate the threshold variant, the inputs to the MPC protocol are modified. By the definition of the MPC protocol, there exist linear functions:
\begin{align}
  \varphi^1_{r,\varepsilon}                 & : (s_A, P, Q, a, b, c) \mapsto (\alpha, \beta), \\
  \varphi^2_{r, \varepsilon, \alpha, \beta} & : (s_A, P, Q, a, b, c) \mapsto v,
\end{align}
such that each party locally computes $\varphi^1$ and $\varphi^2$ on the input shares $\sh{s_A}_i, \sh{P}_i, \sh{Q}_i, \sh{a}_i, \sh{b}_i, \sh{c}_i$, producing $\sh{\alpha}_i, \sh{\beta}_i, \sh{v}_i$ as valid sharings of $\alpha, \beta, v$.

We define the \texttt{input\_plain} as the tuple containing the MPC inputs:
\begin{align*}
...
\begin{align}
  \sh{\texttt{input\_plain}}_i & = \text{Share}(i, \texttt{input\_plain}, \texttt{input\_coef}_1, \dots, \texttt{input\_coef}_\ell) \label{sdith-mpc-share} \\ \nonumber
...
\begin{protocol}[SD-in-the-Head ZK-POK protocol]
  ...
  \begin{enumerate}[parsep=0pt, itemsep=0pt, topsep=0pt]
    \item The prover generates the input sharings and commits to the parties' shares.
    \item The verifier challenges the prover with the randomness \(r, \varepsilon\) for the MPC protocol.
    \item The prover executes the MPC protocol (in their head) and sends the broadcast values to the verifier.
    \item The verifier challenges the prover to open all the parties in \(I \subset [1 : N]\).
...
The soundness of the ZK-POK protocol depends on the binding property of the commitment scheme and the inability of the prover to simulate multiple inconsistent views of the MPC protocol. By employing hash consistency checks and commitment verification, the ZK-POK protocol ensures that a cheating prover cannot deviate significantly from honest execution.
...
The threshold approach slightly degrades the soundness of the protocol. To mitigate this, \textit{parallel repetition} is employed to strengthen the protocol against soundness error. Specifically, the SD-in-the-Head protocol repeats the underlying ZK-POK protocol $\tau$ times, where:
\begin{align*}
...
To accommodate the repetitions, we incorporate a slight modification inspired by the Limbo proof system~\cite{aguilarsyndrome11,delpech2021limbo}. Specifically, we reuse the same MPC challenges $r$ and $\varepsilon$, as well as the Beaver triples, across all $\tau$ repetitions. This tweak allows us to modify the MPC computation input so that $\textit{broad}$ only needs to be computed once.

\subsubsection{Fiat-Shamir transform}
Next, we apply the \textit{Fiat-Shamir heuristic} to transform the challenges of the ZK-POK protocol into a non-interactive form. This is achieved by having the prover derive the challenges directly from the commitments of the MPC inputs and the outputs of the MPC computation.

Additionally, we incorporate the message into the second hash, enabling the pre-computation of the first part of the signature independently of the message. This optimization improves efficiency by allowing some computations to be performed beforehand.
...
\begin{protocol}[SD-in-the-Head Signing Protocol]
  \setlength{\parindent}{0pt}
  \setlength{\parskip}{5pt}
  \titlespacing*{\paragraph}{0pt}{1pt}{1em}
  Given a public SD instance $pk = (H, y)$, secret $sk = (s_A, Q, P)$, message $m \in \{0,1\}^*$. Assume

  \begin{itemize}[itemsep=0pt, topsep=0pt, parsep=0pt]
    \item security parameters $\lambda, \tau$
    \item a $(\ell + 1, N)$-threshold MPC protocol $\varphi^1, \varphi^2$ (\autoref{def:sdith-mpc}, \autoref{sec:mpc})
    \item secure hash functions $\texttt{Hash}_n(d) \rightarrow \texttt{Hash}(n || d)$ (\autoref{sec:prelim_hash})
    \item and random oracle $\texttt{PRG}$ (\autoref{sec:prelim_hash})
    \item \texttt{XOF} (\autoref{sec:xof})
            ...
            \item compute input shares for $i \in [N]$
                  \begin{align*}
                    ...
                    \begin{cases}
                      ...
                      \text{Share}(  i, \texttt{input\_plain},  \\
  ...
  \begin{enumerate}[itemsep=0pt, topsep=0pt, parsep=0pt]
    \item Compute plain broadcast $\texttt{broad\_plain} \stackrel{\varphi^1}{\longleftarrow} \texttt{input\_plain}, r, \varepsilon$
    \item For $e \in [\tau]$, compute broadcast shares
          \begin{align*}
            \texttt{broad\_coef}_j^e \stackrel{\varphi^1, \varphi^2}{\longleftarrow} \texttt{input\_coef}_j^e, r, \varepsilon \quad \text{for each } j \in \ell
          \end{align*}
  \end{enumerate}
  ...
  \paragraph{Phase 5} Building of the signature output the signature $\sigma$.
  \begin{enumerate}[itemsep=0pt, topsep=0pt, parsep=0pt]
    \item Compute authentication paths for each $e \in [\tau]$
          \begin{align*}
    ...
    \item Build the signature
          \begin{align*}
            \sigma = & m, \texttt{salt}, h_1,                                                            \\
                     & \texttt{broad\_plain},                                                            \\
                     & \{\texttt{broad\_coef}^e_1, \dots, \texttt{broad\_coef}^e_\ell \}_{e \in [\tau]}, \\
                     & \{\texttt{auth}^e \}_{e \in [\tau]},                                              \\
                     & \{\sh{\texttt{input}}_i^e\}_{e \in [\tau], i \in I},                              \\
                     & I  \
          \end{align*}
  \end{enumerate}
...
\todo{We can truncate beaver triples as they can be calculated from the input through inverse MPC}

\todo{Talk about inverse MPC instead of interpolation}
...
After creating all the subroutines, we implemented the general signing algorithm described in \autoref{sec:sdith-signature}.
We started by implementing all the sampling needed. This entailed creating the $H'$ matrix, the beaver triples and then defining based on the secret key, the inputs to the MPC simulation.
This led us to create a function to compute the input shares based on the PRG and the plain input shares.
\begin{minted}{rust}
    pub fn sign_message(
        entropy: (Seed, Salt),
