...
\section{Post-Quantum Cryptography and the NIST call to action}\label{sec:quantum}
...
\chapter{Mathematical preliminaries}\label{ch:mathprelim}
...
% subsection  (end)
\subsection{Galois Finite Field}\label{sec:gf256}
...
For polynomial evaluation in the multiparty computation seen in \autoref{sec:mpc}, we need to evaluate a polynomial $P(x)$ at a point $r \in F_q^\eta$. This is done by evaluating the polynomial at each coefficient and summing the results. The polynomial $P(x)$ is defined as
\begin{align}
...
In the following section we will introduce the cryptographic protocols that provide the building blocks used to build the SD-in-the-Head scheme.

\subsection{Commitment Scheme (Merkle Tree)}
A commitment scheme is a way for a prover to commit to a value, and then later prove that it's still the same value as was initially committed to. Such that the following 2 properties are upheld:
\begin{itemize}
  \item \textbf{Binding}: The prover cannot change the value after it has been committed to.
a  \item \textbf{Hiding}: The value is hidden from the verifier until the prover reveals it.
\end{itemize}
One such variant which allows for partial opening, is the merkle tree. 
Let $v_1, \dots, v_n$ be the values being committed to. And let $h$ be a cryptographic PQC secure hash function.
The leafs $a_{0,1},\dots, a_{0,n}$ of the tree is the initial hash values $h(v_1), \dots, h(v_n)$. Every parent $a_{1,0},\dots a_{h,j}$, here $h$ is the height of the tree and $j$ is the number of parents per layer in the tree, is the hash of its children $h(h(v_1)|h(v_2)),\dots, h(h(v_{n-1})|h(v_n))$. The root of the tree $a_{h,0}$ is the commitment value, which is sent to the verifier. Now to open a value $v$, the prover computes the authentication path for $v_p$, where $p$ is the index of the chosen value. The authentication path $A$ for any leaf $a_{0,i}$ is the accumulated neighboring nodes from the leaf up to the root. So the first node $A_0$ is going to be the neighboring node $a_{0,p-1}$ if $p$ is odd or $a_{0,p+1}$ if $p$ is even. This node is going to be the $auth_0$. For the rest of the path $A_i = h(A_{i-1} | auth_{i-1})$ where $auth_i$ is the neighboring node of $A_{i-1}$ at the same level. 
The prover sends $A$ and the index $p$ for the chosen value to the verifier whenever he wants to open a value. The verifier can check that the commitment is correct by recomputing the root using the authentication path and the leaf value.

Due to the use of a PQC secure and collision-resistant hash function the commitment ensures the \textbf{Hiding} property. This comes from the fact that only the root of the tree is shared and if the adversary wants any information on the initial values, she would have to find collisions for the hash function. The same argument holds for the \textbf{Binding} property, as the adversary would have to find a collision for the hash function to change the commitment value.

A nice property of the merkle tree is that you can open a partial set of the committed values by only supplying the authentication path that is needed for the subset of values to be opened. And only sharing a global root values. This is very useful in the SD-in-the-Head threshold protocol, where we only want to open a subset of the committed values.
...
\subsection{Fiat-Shamir Heuristic}\label{sec:fiatshamir}
...
Given a semi-honest $\ell$-private MPC protocol $\pi_f$, a relation $\mathcal{R}$ for some public statement $x$, a witness $w$ and a\footnote{See \autoref{sec:mpc} and \autoref{sec:zk}}. Let $w_i$ be an additive secret share of $w$ for the party $P_i$ so that $w = w_1 \oplus \cdots \oplus w_n$ (this is further addressed in \autoref{sec:lsss}). Let $f$ be a $n$-party functionality $f(x, w_1, \dots, w_n) = \mathcal{R}(x, w)$, s.t. $f(x,w)$ accepts if $(x,w) \in \mathcal{R}$. The ZK protocol proceeds as follows~\cite{ishai2007zero}:

\begin{enumerate}
  \item The prover picks random $w_1, \dots, w_n$ s.t. $w = w_1 \oplus \cdots \oplus w_n$. The prover then:
        \begin{itemize}
          \item Computes the outputs of $\pi_f$ on the inputs $x, w_1, \dots, w_n$ and the randomness $r_1, \dots, r_n$.
          \item Prepares views $V_1, \dots, V_n$ of the parties in the protocol $\pi_f$.
        ...
        \end{itemize}
  \item The verifier picks two random distinct indices $i, j \in [n]$ and sends them to the prover.
  \item The prover opens the commitments to $V_i, V_j$ and sends the openings to the verifier.
  \item The verifier accepts if and only if:
        \begin{enumerate}
          ...
          \item\label{prop:mpcinth_consistent} The two views are consistent according to the public input $x$.
          \item\label{prop:mpcinth_knowledge} Both views output $1$ according to $\mathcal{R}$ meaning that $\mathcal{R}(x,w) = 1$.
        \end{enumerate}views are valid according to the commitment scheme and the two views are consistent according to the public input $x$.
\end{enumerate}

The framework provides the following properties:

\begin{itemize}
  \item \textbf{Completeness}: Given an honest prover, $\mathcal{R}(x,w) = 1$, the correctness of $\pi_f$, \ref{prop:mpcinth_consistent} and \ref{prop:mpcinth_commit} 
  \item \textbf{Soundness}: If $(x,w) \notin \mathcal{R}$, the verifier rejects with overwhelming probability.
  \item \textbf{Zero-Knowledge}: The verifier learns nothing about the inputs of the other parties beyond what can be inferred from the output of the function.
\end{itemize}
...
\subsection{Zero-Knowledge proofs from MPC}\label{sec:zkmpc}
...
\section{Secret Sharing Schemes}\label{sec:lsss}
...
\subsubsection{Multiplication and Beaver Triples}\label{sec:beaver}

\subsection{Verification of a multiplication triple using another}\label{sec:verify}
...
\subsubsection{Addition of parameters}
Then we introduced the parameters from the spec. f4739166d684

\subsubsection{Galois field arithmetic}
\label{}
...
\subsection{Comparison with spec impl}\label{sub:comparison_with_spec_impl}
...
\subsection{Array structure}\label{sub:array_structure} % (fold)
...
\subsection{Rayon}\label{sub:rayon} % (fold)
...
\subsection{Kat for NIST}\label{sub:kat_for_nist} % (fold)
