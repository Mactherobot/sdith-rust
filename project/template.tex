...
Given a semi-honest $\ell$-private MPC protocol $\pi_f$, a relation $\mathcal{R}$ for some public statement $x$, a witness $w$ and a\footnote{See \autoref{sec:mpc} and \autoref{sec:zk}}. Let $w_i$ be an additive secret share of $w$ for the party $P_i$ (this is further addressed in \autoref{sec:lsss}). Let $f$ be a $n$-party functionality $f(x, w_1, \dots, w_n) = \mathcal{R}(x, w)$, s.t. $f(x,w)$ accepts if $(x,w) \in \mathcal{R}$. The ZK protocol proceeds as follows~\cite{ishai2007zero}:
