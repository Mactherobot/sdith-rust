...
\begin{protocol}[SD-in-the-Head Verification Protocol]\label{def:sdith-verify}
  ...
  \paragraph{Assumptions}
  \begin{enumerate}[itemsep=0pt, topsep=0pt, parsep=0pt]
    \item $(\ell + 1, N)$-threshold MPC protocol with inverse computation $\phi^1, \phi^2$ (\autoref{def:sdith-mpc}, \autoref{thm:mpc_inverse}, \autoref{sec:mpc}), \autoref{equation:input_sharing_inverse}
...
To allow for our implementation to be used in other projects, we have created it as a create. This is done creating a \rust{lib.rs} file within the root folder of the project.
