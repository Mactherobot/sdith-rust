...
\begin{protocol}[SD-in-the-Head Signing Protocol]
  ...
  \begin{enumerate}[itemsep=0pt, topsep=0pt, parsep=0pt]
    \item Compute the second hash
          ...
          \end{align*}
    \item Compute the second challenge $I \subset [N] \leftarrow \texttt{PRG}(h_1)$ for $|I| = \ell$
  \end{enumerate}
...
\end{minted}
As this relies on the PRG to sample the values, we have a test that ensures that the values are unique and within the correct range.
...
We sepee
\todo{Write about the expand view challenge hash, and the compute share methods}
