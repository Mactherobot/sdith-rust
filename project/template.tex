...
One of the reasons for choosing Rust is that it provides several tools and features that make it easier to write maintainable and readable code. In this section, we will discuss some of the features that we utilized in our implementation to achieve this goal.
...
Often when implementing a program you end up creating functionality that you want to reuse. However, due to the type system or the structure of the program, you might end up writing the same code multiple times. Rust provides two features that help with this: \textit{Generics} and \textit{Traits}.
...
\end{minted}
Due to the type system, you cannot call the same function for different types. So would  have to resort to duplicating the code for \rust{u16} and splitting it into two functions, \rust{add_u8} and \rust{add_u16}. This leads to several issues, especially when maintaining a large codebase. For example, if we update one function, we manually have to update the other. As a solution, Rust allows us to define generic functions.
\begin{minted}{rust}
  fn add<T>(a: T, b: T) -> T where T: Xor {
    a ^ b
  }
\end{minted}

\subsubsection{Linting and Documentation}
\todo{mention the \rust{#![warn(missing_docs)]} linting rule and how to document code with \rust{///} and \rust{//!}} 

\subsection{Dynamic compilation}
\todo{describe constants as a lead in to build script and feature flags}

Introduce the issue: Changing constants. Maybe state it as a general problem first, then turn to our specific problem.

Second: opt-in optimisations. We use it to allow bench marking.

\subsubsection{Build script}
A script that runs before compile. Give use cases: We have our own of generating constants. Mention the \texttt{build.rs} file. Show we use it to generate constants. Point: It is a neat way of generating constants. Maybe see of they mention more use cases.

\subsubsection{Feature flags}
Describe how you can parse feature flags in Cargo.toml (how to set them for packages and how to set our own.). Point: Its a neat way of defining build configurability.

\subsubsection{Configuration conditional checks \texttt{\#[cfg(...)]}}
Show how you can dynamically tell the compiler which code to add to the binary. You can pose the simple example of tests.

You can mention the \texttt{cfg!} macro for inlining if statements.
