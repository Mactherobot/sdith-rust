...
\todo{Add some examples of the significant advancements in quantum technology. Willow chip from google}
...
In proving the security of hash functions, researchers often resort to models like the Quantum Random Oracle Model (QROM), this approach however suffers form the fact that it abstracts the hash functions as ideal even when evaluated in a superposition. While there is significant work being done to further prove the future security of hash functions~\cite{dtuPostquantumSecurity}.
\todo{Read through the article i sent to Diego}
...
\subsubsection{Quantum Random Oracle Model (QROM)}
...
mod tests {
  #[test]
  fn test_field_arith_u8_definitions() {
...
The field extensions were built using a \textit{tower structure} as we first implemented the base field $\mathbb{F}_{q}$ for $q=2^8$ -- also called Rijndael's field. Then, we proceeded to implement trait for the first extension from \rust{u8} to \rust{[u8; 2]} in $\mathbb{F}_{q^2}$ and next to $\mathbb{F}_{q^4}$.

\subsubsection{$\mathbb{F}_{q^4}$ Tower construction}\label{sub:first_simple_extension}
...
\begin{align}
  (a + bX) \cdot (c + dX) & = ac + (ad + bc)  X + bd  X^2                        \\
                          & = ac + (ad + bc)  X + bd  (X + 32)                   \\
...
\subsubsection*{Optimal Building Block}
...
\subsubsection*{Testing the $\mathbb{F}_{q^2}$ Field}
...
\subsubsection{Extending to the 32 bit field $\mathbb{F}_{q^4}$}\label{sec:gf_256_32_bit_field} % (fold)
\todo{Can we remove this section and only discuss the testing? Its technically the same but uses the multiplication with 32 function from the previous field}
Now that we have the $\mathbb{F}_{q^2}$ field we can implement the $\mathbb{F}_{q^4}$ field arithmetic as an extension. We represent an element in the $\mathbb{F}_{q^4}$ field as a 4-byte array \rust{type FPoint = [u8; 4]}. So we have a representation

\begin{align}
  F_{q^4} = F_q[Z] / (Z^2 + Z + 32(X))
\end{align}

The implementation of the next extension is similar to the previous as we simply repeat the same extension on the $\mathbb{F}_{q^2}$ field.

\subsubsection{Addition in $\mathbb{F}_{q^4}$}

First we implemented the addition which is again a simple XOR operation on each component. Using the feature flag we created 2 implementations, first a simple addition using the $$\mathbb{F}_{q^2}$$ field and then a SIMD implementation using the \texttt{std::simd} module, which yielded a little more performance.
\begin{minted}{rust}
#[inline(always)]
...
\subsubsection{Multiplication in $\mathbb{F}_{q^4}$}
...
\subsubsection{Testing the $\mathbb{F}_{q^4}$ Field}
...
After defining the field arithmetic, we implemented the necessary vector operations for the $\mathbb{F}_{q^4}$ field. These operations are essential for the polynomial interpolation and MPC computations in the protocol.
