...
\begin{definition}[ZK from Cut-and-Choose (Multiplication)]\label{def:cut-and-choose}
  Denote the security parameters $M$ the number of pre-processing executions, $N$ the number of parties, $\tau$ the security parameter, a secure hash function \autoref{sec:prelim_hash} and a \textit{Random Oracle-based commitment scheme}. Given a $(N-1)$-private MPC protocol $\Pi_C$ for a circuit $C$ and an ideal functionality $\mathcal{F}$. Furthermore, we also denote $\mathcal{P}$ to be the prover and $\mathcal{V}$ as the verifier. \textit{Note} that for simplification we assume that there is only one multiplication gate in the circuit, to align with our purposes. The paper by~\cite{baum2020concretely} provides a more detailed protocol for multiple multiplication gates and square gates.
  \begin{enumerate}[itemsep=0pt, parsep=0pt]
    ...
    \begin{enumerate}[nolistsep]
      ...
      \item $\mathcal{P}$ chooses a master seed $sd_e$ and  expands $sd_e$ to $sd_{e,i}$ for each $i \in [N]$ using $\Pi_C$.\footnote{If the seed is expanded in a correlated way, you can send just $sd_e$ and have the verifier recreate the expanded seeds accordingly}
...
Building on the protocol outlined earlier, the authors propose a method based on sacrificing multiplication triples, as described in \autoref{def:sacrifice}, to verify the correctness of multiplication gates. The following lemma establishes that the probability of successful cheating is bounded by $\frac{1}{|\mathbb{F}|}$:

\begin{lemma}\label{lem:sacrifice}
  If $\sh{a}, \sh{b}, \sh{c}$ or $\sh{x}, \sh{y}, \sh{z}$ is an incorrect multiplication triple, then the parties output \texttt{Accept} in the sub-protocol with probability at most $\frac{1}{|\mathbb{F}|}$.
\end{lemma}
\todo{Maybe insert proof? It is kinda simple.}
This lemma provides a lower bound on the probability of a malicious party $\mathcal{P}$ successfully cheating. Consequently, the security parameters can be reduced without compromising security, thereby improving the efficiency of the protocol.

The primary modification lies in Step 3, where the verifier $\mathcal{V}$ selects a random subset $E \subset [M]$ to verify the correctness of the multiplication triples generated by the prover $\mathcal{P}$. In contrast, the sacrificing protocol uses random $\epsilon$ values, chosen by $\mathcal{V}$, which the prover $\mathcal{P}$ uses to commit to the verification of the multiplication gates. This adjustment enables the verifier to independently verify the correctness of the triples without relying on the prover to generate valid triples.
