...
To ensure memory safety, Rust uses a borrow checker to prevent data races and memory leaks. We will now dive into the different aspects of how the borrow checker works in Rust. We will only cover the most important aspects off the borrow checker. More in depth information can be found in the official documentation~\cite{rustlangRustProgramming}.
...
\begin{minted}{rust}
  ...
  let v = d;
\end{minted}
\subsubsection*{Behavior of the String Literal \(d\)}

\begin{itemize}
  \item \textbf{Case 1:} If the string literal \(d\) implements the \textit{Copy} trait, its value is simply copied onto the stack. This means \(d\) remains accessible after being used or assigned elsewhere because its data was duplicated.

  \item \textbf{Case 2:} If \(d\) is instead a \textit{pointer} to a heap-allocated string, it resides on the stack but points to data stored on the heap. When \(d\) is moved to another variable (e.g., \texttt{v}), the pointer itself is transferred to \texttt{v}. The heap-allocated data remains unchanged, but the ownership of the pointer shifts to \texttt{v}.
\end{itemize}

\subsubsection*{What Happens After a Move?}

\begin{itemize}
  \item \textbf{In Case 1:} Since \(d\) implements the \textit{Copy} trait, it is unaffected by the move. Copies of \(d\) exist independently on the stack, so using \(d\) after the move is perfectly safe.

  \item \textbf{In Case 2:} Here, the pointer in \(d\) is moved to \texttt{v}. After this, \(d\) is no longer valid. Any attempt to use \(d\) would result in a compile-time error because \(d\)'s ownership has been transferred. This ensures \textit{memory safety} and prevents \textit{data races}.
\end{itemize}

\subsubsection*{Why the Restriction?}

Allowing \(d\) to be used after the move would lead to undefined behavior. For instance, if multiple pointers were allowed to simultaneously access or modify the heap-allocated data, it could result in \textit{data races}. To avoid such risks, Rust enforces strict ownership rules. If you need to continue using \(d\) after a move, you must explicitly \textit{clone} the heap data before transferring ownership. This approach encourages developers to carefully manage how data is accessed and modified, promoting safe and predictable memory usage.

\subsubsection{Ownership and Functions}\label{sec:ownership_and_functions} % (fold)
Now that we have covered the basics of ownership, let's explore how it interacts with functions. When passing a value to a function, the function takes ownership of the value. This means that the function will manage the lifetime of the value, ensuring that it is not used after the function call, unless copied. A function can also return ownership of a value, allowing the caller to manage the lifetime of the returned value. Here's an example:
\begin{minted}{rust}
  fn take_and_give(d: String) -> String {
    d
  }
\end{minted}
In this example, the function \texttt{take\_and\_give} takes ownership of the \texttt{String} value \texttt{d} and returns ownership of the same value. But having to return ownership of values can be cumbersome, and gives a lot of boiler plate code:
\begin{minted}{rust}
  fn main() {
    let v1 = vec![1, 2, 3];
  ...
  fn sum_vector(v1: Vec<i32>) -> (Vec<i32>, i32) {
    let mut result = v1.iter().sum();
    (v1, result)
  }
\end{minted}
So how can we avoid this boilerplate of returning ownership? Rust provides a way of avoiding this transfor of ownership by using \textit{references}. 
% subsubsection Ownership and Functions (end)

\subsubsection{Borrowing and references}
We saw how ownership works in the previous section, ensuring memory safety. However, there were some caviats for functions. In the previous example, the function \texttt{take\_and\_give} took ownership of the \texttt{String} value \texttt{d} and then returned ownership of the same value. This is fine, but it can be cumbersome to write functions that take ownership of values. To avoid this, Rust provides a feature called \textit{references}. A reference is a pointer to a value that does not own the value. Instead, it borrows the value for the duration of the reference. Here's an example:
\begin{minted}{rust}
  fn main() {
    let v1 = vec![1, 2, 3];

    let sum = sum_vector(&v1);
    // v1 is now still valied because we borrowed it
    println!("The sum of the vector is: {}", sum);
  }
  
  fn sum_vector(v1: &Vec<i32>) -> i32 {
    let mut result = v1.iter().sum();
    result
  }
\end{minted}
If we however, also wanted to mutate the vector, we could have done so by using the \texttt{mut} keyword to borrow a mutable reference. This would allow the function \textit{sum\_vector} to modify the vector.

We have used this extensively in our implementation to ensure memory safety, but also to ensure that we optimize the code for performance and memory usage. We will go more into how we have used this when describing our architecture.
...
\todo{finish this segment}

\subsubsection{Error handling} % https://www.lurklurk.org/effective-rust/panic.html
Testing that functions return the correct values based on \textit{correct} inputs are a crucial part of ensuring correctness. But what happens if you have unexpected inputs, maybe from the user of a CLI? This can lead to errors. So how do Rust allow us to deal with errors? There are two ways: 
\begin{itemize}
  \item Using a \textbf{panic}: Stopping the execution of the program. 
  \item Returning a \textbf{Result}: A type that wraps the return value with an error value allowing the caller to handle the error.
\end{itemize}
...
The general rule is that, if your code would end up in a bad state from which it can not recover it should panic. Everywhere else, you should return a \textit{Result}.
An example of using the \textit{Result} type is shown below:
\begin{minted}{rust}
...
\end{minted}
There might be a number of reasons why a signature could not be generated, but a simple one that we want to relay to a potential use is that the secret key is invalid. In this case, we would return an error with the \textit{SigningError} variant. If a panic was used the caller of the sign function would not be able to recover from the error.
In our implementation we have used the \textit{Result} type to handle errors. This functionality is similar to exceptions in other languages, allowing you to handle errors gracefully and provide meaningful error messages. As well as making our code reliable in the face of inevitable problems.
