...
To create a public and private key pair, we need a witness for the ZK in the form of a syndrome decoding (SD) instance. As outlined in \autoref{eq:polynomial_representation}, this involves generating a set of polynomials derived from a random $x$ vector in the syndrome instance.

\subsubsection{Sampling the $x$ vector}

We needed to sample an $x$ vector with the required $d$-split hamming weight $w$ and length $m$. The function \rust{sample_non_zero_x_positions} determines the positions of the non-zero coordinates. It samples random coordinates within the range of $x$ until $w$ non-zero coordinates are selected. The function returns the $x\_vector$ and the positions of the non-zero coordinates, which are used to compute the polynomial $Q'$.
...
The polynomial $Q'$ is computed based on the positions of the non-zero coordinates in $x$. We iterate through the positions to compute the coefficients of $Q'$. The computation of each coefficient follows the method described in \autoref{sec:polynomial_representation}.
...
Additionally we tested the correctness of the whole polynomial relation generation process by verifying that the relation was sound and that $S \cdot Q = P \cdot F$.
