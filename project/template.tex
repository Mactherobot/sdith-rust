...
Next, we compute $\ell$ uniformly random coefficients $\texttt{input\_coef} \in \mathbb{F}_q^{|\texttt{input}|}$ and determine the sharing:
\begin{align*}
  \sh{\texttt{input}}_i = \texttt{input} + \sum_{j=1}^\ell i^j \cdot \texttt{input\_coef}_j
\end{align*}
...
\begin{align*}
  (\sh{\alpha}_i, \sh{\beta}_i, \sh{v}_i) = \texttt{broad} + \sum_{j=1}^\ell i^j \cdot \texttt{broad\_coef}_j
\end{align*}
...
\begin{protocol}
  ...
  \begin{itemize}[itemsep=0pt, topsep=0pt, parsep=0pt]
    ...
    \item a $(\ell + 1, N)$-threshold MPC protocol \texttt{MPC} (\autoref{def:sdith-mpc}, \autoref{sec:mpc}) and set $S \subset [N]$ s.t. $|S| \geq \ell + 1$
  ...
  \begin{enumerate}[itemsep=0pt, topsep=0pt, parsep=0pt]
    ...
    \item For each iteration $e \in [\tau]$ compute random sharings of the secret and valid Beaver triples for each $j \in [t]$
          \begin{align*}
            \texttt{XOF} \rightarrow \sh{sk}^{e}, \sh{a}^{j, e}, \sh{b}^{j, e}, \sh{c}^{j, e}
          \end{align*}
    \item Compute the commitments for $i \in [N]$:
          \begin{align*}
            \texttt{com}_i^{e} := \texttt{Hash}_0(\texttt{salt}, e, i, \sh{sk^{e}}_i, \sh{a}^{j, e}_i, \sh{b}^{j, e}_i, \sh{c}^{j, e}_i) \quad j \in [t]
          \end{align*}
