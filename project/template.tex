...
A good example is wanting to build and link some C code:
\begin{minted}{rust}
  fn main() {
...
\end{minted}
This could allow the use of C code in the Rust project. This can be especially useful when trying to use parts of a reference implementation in C or C++. If parts are already heavily optimized.  
\todo{Write about woulda coulda shoulda but didnta}

\subsubsection{Feature flags}
Rust has a crisp feature flag system that allows for conditional compilation. This allows for a lot of flexibility when it comes to writing code that is optimized or configured for different use cases. A good example is different computer architectures, this stems from the fact that different architectures does not share the same instruction sets, making code for one architecture unusable on another. 
Another great example is the one mentioned before about different sets of arithmetic fields. To solve the issue posed about having to create a struct in order to hold functionality for the same type we can instead use feature flags to conditionally compile the code to the specific field that is needed. This is done by adding the following to the \texttt{Cargo.toml} file:
\begin{minted}{toml}
[features]
  default = ["gf256"]
  gf256 = []
