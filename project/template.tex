...
Secure Multi-Party Computation (MPC) refers to a cryptographic protocol enabling multiple parties to jointly compute a function represented by a circuit $C$, ensuring that no information about the individual inputs is revealed beyond what can be inferred from the output of $C$. These protocols can be further enhanced to guarantee correctness, even in scenarios involving malicious adversaries.The MPC-in-the-Head (MPCitH) framework, introduced by~\cite{ishai2007zero}, builds upon these techniques to construct generic zero-knowledge protocols (ZK). A ZK protocol allows a \textit{prover} to convince a \textit{verifier} of the validity of a statement without revealing any information about the inputs to the statement. The framework provides a versatile method of constructing protocols that are quantum safe as it relies on assumptions that are still believed to be quantum safe. Namely, commitment schemes and hash functions~\cite{feneuil2023threshold}.

The paradigm functions by letting the \textit{prover} emulate an $\ell$-private MPC protocol\footnote{i.e $\ell + 1$ shares are needed to reconstruct the input of the circuit} "in the head", while the \textit{verifier} challenges the prover to reveal $\ell$ random shares. By the definition of the MPC protocol, the input remains hidden, ensuring zero-knowledge.
