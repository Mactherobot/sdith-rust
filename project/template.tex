...
\begin{enumerate}[parsep=0pt, itemsep=0pt]
  ...
  \item Alice computes her commitment $c_A = h(g | s_r)$ for some random string $s_r$ and sends $c_A$ to Bob.
  ...
  \item Alice announces $g', s_r$ to Bob.
  \item Bob accepts the result if $h(g | s_r) = c_A$.
\end{enumerate}

From \autoref{lem:preimage}, we can see that Bob is not able to learn $g$ before he flips his coin without knowing $g \mid s_r$. We can adjust the length of $s_r$ to enhance the security of the protocol.

Furthermore, by \autoref{lem:collision}, Alice cannot change her result $g$ without being able to find a different $s_r'$ such that:
\[
h(g' \mid s_r') = c_A.
\]
