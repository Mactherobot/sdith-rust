...
\todo{When the specification is written write up the beginning for this.}
When implementing the merkle tree we ran into a few issues due to the nature of the implementation. We decided to do it ourselves such that it fit with the specification. 
This implementation was done using a single array to hold the whole tree, where the last 256 items in the array was the commitments. This made it so that when implementing the different methods we had to be careful and use the correct indexes. This proved to be quite difficult to keep track of. Furthermore, we decided to do the same optimization as was done in original SDITH implementation in C. 
