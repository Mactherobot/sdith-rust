...
To obtain a signature scheme from the MPC protocol \autoref{def:sdith-mpc}, two transformations are needed. First, into a zero-knowledge proof of knowledge (ZK-POK) using MPCitH and second, into a signature scheme using the Fiat-Shamir transform.

The ZK-POK is based on the verification by sacrificing triples strategy for MPCitH~\cite{baum2020concretely} detailed in \autoref{sec:mpc_sacrificing}.

\begin{protocol}[ZK-POK from MPCitH~\cite{feneuil2022syndrome}]
  ...
  \begin{itemize}
    \item $\mathcal{P}$ computes the proof witness $(Q, S, P)$ from $x$ and public polynomial $F$.
    \item $\mathcal{P}$ Sample a root seed $sd$ and expand to seeds $sd_i$. Commit to the party randomness $\texttt{com}(sd_i, p_i)$.
    \item $\mathcal{P}$ prepares inputs for the MPC protocol. For each party $i \in [1 : N]$ and for all $j \in [t]$, it computes
    \begin{itemize}
      \item 
      \item $\mathcal{P}$ generates the inputs to the MPC protocol
      \begin{itemize}
        \item \textit{Beaver triples}: $\sh{a}, \sh{b} \leftarrow \texttt{PRG}(seed_i)$ and $\sh{c} = \sh{a} \cdot \sh{b}$.
        \item $state_i = (seed_i, \sh{x_A}, \sh{Q}, \sh{P})$.
      \end{itemize} 
      \item $\mathcal{P}$ commites 
    \end{itemize}
...
\paragraph{Functions and ownership}\label{sec:rustlifetimes} % (fold)
...
\paragraph{Error handling}\label{sec:rusterror} % https://www.lurklurk.org/effective-rust/panic.html
