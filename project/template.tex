...
After the implementation of all subroutines, we proceeded to develop the signing algorithm as described in \autoref{def:sdith-sign}. The method was designed to align with and clearly follow the prescribed algorithm in order to facilitate understanding.
...
The final piece of the puzzle was implementing the verification algorithm. Following the specification, we utilized the KAT-generated signatures to validate that we could correctly parse and verify a signature. This process was designed to ensure the accuracy of our signature parsing, inverse MPC computation, and Merkle tree verification. However, the initial attempt did not succeed, prompting us to compare our implementation with the specification as detailed in \autoref{sub:comparison_with_spec_impl}.
...
Talk about how it was to work with the spec

-  We had trouble aligning with the reference input and output due to specific implementation details (PRG setup)
- The order of the PRG calls are important when comparing the resulting outputs
- Missing a deeper clarification on the reasoning behind the offset parameter in the MPC protocol.
