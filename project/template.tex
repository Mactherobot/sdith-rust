...
Pre-processing in MPC is an optimization technique that leverages shares of correlated randomness, such as randomness derived from a shared seed, which remains independent of the inputs to the protocol. This independence allows the parties to accelerate computation and perform pre-computation in advance. As a result, this approach allows the development of MPCitH protocols that can leverage greater soundness while having lesser impact on the size of the proof. Such a technique was introduced by~\cite{katz2018improved} as part of their \textit{cut-and-choose} approach. 

\subsubsection{MPC over arithmetic circuits}\label{sec:zk-arithmetic-mpc-circuits}
First we need to describe a basic MPC protocol that allows us to securely compute an arithmetic circuit. While addition is considered generally easy for a MPC protocol, multiplication is more challenging. 

In order to form a ZK proof for the SD polynomial relation in \autoref{sec:polynomial_representation}, we need a MPC protocol that allows us to securely compute multiplication circuits. For this we use \textit{beaver triples}.
...
\begin{definition}[Multiplication circuit using beaver triples]\label{def:beaver-multiplication}
  Given a preprocessed random beaver triple $\sh{a}, \sh{b}, \sh{c} $, the parties do the following:
  \begin{enumerate}
  ...
  The above is a well-known technique \cite{beaver1992efficient} and works because
  \begin{align*}
...
Building upon the multiplication protocol above, Baum and Nof~\cite{baum2020concretely} outlines the fundamental concept of the cut-and-choose technique in their \textit{HVZKAoK Protocol using Cut and Choose}. For a detailed outline of the protocol see \cite[s3.2]{baum2020concretely}. Here we provide a high-level overview of the protocol.\\
...
\subsubsection{Zero-Knowledge from Sacrificing}\label{sec:zk-sacrifice}

Building on the protocol outlined earlier, the authors propose a variant. In this variant, instead of simulating the computing of a value, the protocol instead simulates a \textit{verification} of the triple relation by sacrificing another triple. Consider the protocol below based on the MPC protocol described in~\cite{damgaard2012multiparty}.
...
\subsubsection*{The Sacrificing Protocol}

The Sacrificing Protocol starts as the previous protocol as $\mathcal{P}$ generates triples and input shares and commits to them. The primary modification from the Cut-and-Choose protocol is in \hyperref[cac:round2]{round 2}. Instead of challenging the triples generated by the prover, th protocol has $\mathcal{V}$ generate a random seed to produce random $\epsilon$ values to be used by the MPC protocol from \autoref{def:sacrifice}. The prover $\mathcal{P}$ uses to the seed to regenerate the $\epsilon$ values and commit to the verification of the multiplication gates in the MPC protocol. This adjustment enables the verifier to independently verify the correctness of the triples without relying on the prover to generate valid triples.

\subsubsection*{Soundness}

We bound probability that $\mathcal{V}$ accepts while $C(w) \neq y$. $\mathcal{P}$ can cheat by changing the shares of the output wire of a multiplication gate or adjusting the share on one of the input wires to make the output consistent with our assumption. The latter is similar to the Cut-and-Choose protocol and succeeds with $\frac{1}{N}$ probability. We saw from \autoref{lem:sacrifice_soundness} that the probability of tampering with the output wires of the multiplication gates is bounded by $\frac{1}{|\mathbb{F}|}$.

\begin{lemma}[Sacrificing protocol cheating probability]\label{lem:sacrifice_soundness} Let $\xi_{sac}(M,N)$ be the probability that $\mathcal{V}$ accepts while $C(w) \neq y$. Then
  \begin{align*}
    \xi_{sac} (M,N) = \left(\frac{N + | \mathbb{F}| -1}{|\mathbb{F}| \cdot N}\right)^M
  \end{align*}
\end{lemma}

The sacrifice protocol achieves a smaller cheating probability compared to the Cut-and-Choose protocol. This improvement allows the same soundness level for the entire protocol to be achieved with a lower value of \( M \), thereby reducing the overall communication complexity. Baum and Nof~\cite{baum2020concretely} demonstrate that their sacrifice protocol achieves the same bits of security while significantly lowering communication complexity compared to earlier MPCitH protocols like Stern~\cite{stern1996new}. Furthermore, it markedly decreases the prover's runtime compared to Ligero~\cite{ames2017ligero}.

However, additional performance gains have recently been achieved by leveraging threshold secret sharing schemes, as explored in~\cite{feneuil2023threshold}.

\subsection{Linear Secret Sharing Schemes (LSSS)}\label{sec:lsss}

This approach further builds upon the MPC pre-processing approach introduced by Katz et al~\cite{katz2018improved} and Baum and Nof~\cite{baum2020concretely}, but utilises threshhold secret sharing schemes to reduce the computations required by both the prover and the verifier.

"Applying our approach with a low-threshold LSSS also boosts the performance of the proof system by making the MPC emulation cost independent of N for both the prover and the verifier. The gain is
particularly significant for the verification time which becomes logarithmic in N (while the prover still
has to generate and commit the N input shares)."

We change to use LSSS. We have to change the commitment scheme from before where we hashed all values into one hash. Now we will not be sending all values (only $\ell$ values), but we still need to commit to the whole computation -- also the values that $\mathcal{P}$ did not send to $\mathcal{V}$. For this we can use Merkle Tree Commitment (\autoref{sub:merkle_tree_prelim}) scheme which supports partial openings. I.e. we can commit to a the whole computation and the verifier can verify it by opening only a few commitments. 
...
\textit{Linear secret sharing schemes}, or $(+,+)$-homomorphic schemes, refers to secret sharing schemes that are linearly homomorphic over some field $\mathbb{F}$ (say the galois field \texttt{GF256} described in \autoref{sec:gf256}). This means that given shares $\sh{a}$ and $\sh{b}$ we have that
\begin{definition}
...
The protocol requires the use of a commitment scheme to ensure that the right multiplication triples are used in the MPC-in-the-Head subroutines. For this we used the merkle tree as mentioned in the \autoref{spec:merkle_tree}.

\subsubsection{Merkle Tree impl}\label{sub:merkle_tree_impl}
Our merkle tree implementation was done using a sized array as data structure. Meaning that we save all the leafs of the tree in the last 256 values of the array. Allowing the utilization of sized arrays. Here we again used feature flags to allow for different implementations of the merkle tree.
...
% Removed after using feature flags instead
