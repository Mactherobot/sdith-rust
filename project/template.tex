...
\begin{definition}[$(\ell, n)$-private (threshold) SSS]\label{def:mpc-ss-threshold}
  ...
  \begin{itemize}[parsep=0pt, itemsep=0pt]
    ...
    \item \texttt{Reconstruction}: Any $\geq t + 1$ of these shares jointly determine the secret $s$ uniquely (\textit{($\ell + 1$)-opening}).
  \end{itemize}
\end{definition}
...
Shamir's Secret Sharing scheme~\cite{shamir1979share}. For a secret $s$ and a given security threshold $\ell$, the share $\sh{s}$ is generated by sampling a random polynomial of degree $\ell-1$ with $s$ as the free coefficient. Each participant is given a share of the polynomial evaluated at a point. The secret can be reconstructed by interpolating the polynomial from a sufficient number of shares.

The $(\ell,n)$ Shamir's polynomial based secret sharing scheme is $(+,+)$-homomorphic in which the addition of two polynomials secrets equals the Lagrange's interpolation of the sum-of-shares for the same subset of shares.

\todo{Proof of lemma and solution}

In \autoref{sec:lsss} we can see how schemes such as this can be used to dramatically reduce the size of the proofs required to prove the SD relation.
