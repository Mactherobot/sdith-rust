...
A majority of constructions based on the MPCitH framework~\cite{baum2020concretely,feneuil2022syndrome,katz2018improved} are based on $(N-1,N)$ additive SSS. These
...
While a majority of MPCitH frameworks, including the two described above, use additive secret sharing, Feneuil and Rivain~\cite{feneuil2023threshold, feneuil2023threshold2} explore the use of threshold secret sharing schemes \autoref{sec:threshold-sss} instead. This approach further builds upon the MPC pre-processing approach, but the advantage of threshold schemes, denoted $(\ell,n)$-private SSS, in the MPCitH framework resides in much faster computation times as the prover only has to emulate $\ell + 1$ parties and the verifier only has to emulate $\ell$ parties.
