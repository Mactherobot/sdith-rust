...
Our objectives include delivering a high-performance, thoroughly documented implementation, complemented by extensive test coverage to ensure correctness and security. Additionally, we aim to identify and address potential pitfalls and challenges encountered during the implementation process and supply these to the authors. Furthermore, we will be condensing the literature behind the SD-in-the-Head protocol and presenting an in-depth report on the protocol and its prerequisites.
...
A commitment scheme allows a \textit{prover}, Alice, to commit to a value $v$,\textit{heads} or \textit{tails}, and later prove, when Bob has flipped a coin, that the revealed value is identical to the one initially committed to, so if Alice picks \textit{heads} she can not change it after Bob has flipped the coin, and Bob does not know what Alice picked. A simple scheme can be created with a hash function, where the commitment is simply the hash of the value. Now Alice can send this hash value to Bob, neither Bob nor Alice can change the value, and Bob does not learn anything about the value. After Bob has flipped the coin, Alice can reveal the value to Bob. This way none of the parties can cheat in a simple coin flip.
...
A variant of such a scheme which allows for partial opening, meaning that the prover does not have to open the entire commitment, but can instead open a subset of the commitment, is the \textit{Merkle Tree Commitment scheme}.
...
\begin{definition}[Opening a shared value]\label{def:ss-opening}
  Denote \texttt{open}
\end{definition}
 
\begin{definition}[$(\ell, n)$-private secret sharing scheme]\label{def:mpc-ss-threshold}
  A $(t, n)$-threshold secret sharing scheme~\cite{cramer2015secure}, where $t, n$ are integers with $0 \leq t < n$, provides a means to ``disperse'' secret data $s$ into a  sequence of $n$ pieces of data, called \emph{shares}, such that:
\begin{itemize}[parsep=0pt, itemsep=0pt]
    \item Any $\leq t$ shares jointly give no information on the secret $s$ (\textit{t-privacy}).
    \item Any $\geq t + 1$ of these shares jointly determine the secret $s$ uniquely (\textit{(t + 1)-reconstruction}).
\end{itemize}
\end{definition}
...
In order to form a ZK proof for the SD polynomial relation $P \cdot F = S \cdot Q$ (\autoref{sec:polynomial_representation}), we need a MPC protocol that allows us to securely compute an arithmetic circuit. For this we use \textit{beaver triples}.
...
Consider the protocol for a single multiplication gate below by~\cite{baum2020concretely}, based on the MPC protocol described in~\cite{damgaard2012multiparty}.

\begin{definition}[Verification of a multiplication triple by sacrificing another]\label{def:sacrifice}
  Given an input triple $(x,y,z) \in \mathbb{F}$ random shared triple $([[a]], [[b]], [[c]]) \in \mathbb{F}$, it is possible to verify the correctness of the statement $z = x \cdot y$ without revealing any information on either of the input. Define \texttt{open} to denote the opening of a shared value by broadcasting the shares between all parties allowing them to reconstruct the original value.
  \begin{enumerate}
...
Note, that a similar technique can be applied to square relations, we focus here solely on multiplication, as it suffices for the SD relation described in \autoref{eq:polynomial_representation}. \autoref{def:cut-and-choose} outlines the fundamental concept of the cut-and-choose technique. For a more comprehensive protocol, refer to the \textit{HVZKAoK Protocol using Cut and Choose} discussed in~\cite{baum2020concretely}. Note

\begin{definition}[ZK from Cut-and-Choose (Multiplication)]\label{def:cut-and-choose}
  Denote the security parameters $M, N, \tau$ a secure hash function \autoref{sec:prelim_hash} and a \textit{Random Oracle-based commitment scheme}. Given a $N$-private MPC protocol $\Pi_C$ for a circuit $C$ and an ideal functionality $\mathcal{F}$. \textit{Note} that we assume that there is only one multiplication gate in the circuit, to align with our purposes.
  \begin{enumerate}[itemsep=0pt, parsep=0pt]
    ...
    \begin{enumerate}[nolistsep]
      ...
      \begin{enumerate}
        \item To fix the initial randomness of $[c_e]$, recompute $a_e$ and $b_e$ from the shares and set $c_e = a_e \cdot b_e$.
        \item Compute $\Delta_e = c_e - \sum_{i=1}^N c_{e,i}$.
        \item Set the triple of the multiplication gate to ($[a_e], [b_e], [c_e] + \Delta_e$).
      \end{enumerate}
    ...
    \item $\mathcal{V}$ outputs \texttt{Accept} if
          \begin{enumerate}[nolistsep]
            ...
            \item $\mathcal{V}$ verifies for each $e \in \bar{E}$ that the recomputed output of $\Pi_C$ is $y$
          \end{enumerate}
  \end{enumerate}
\end{definition}
...
To re-iterate, we set out to make it so that the communication complexity of the ZK MPCitH protocol was not bounded by $N$. We achieve this by having any communication needed between parties

\subsubsection{Proving a multiplication triple by sacrificing another}\label{sec:sacrifice_beaver}

\todo{define the function \texttt{open}}
