...
We define the \texttt{input} as the tuple containing the MPC inputs:
\begin{align*}
  \texttt{input} = (s_A, P, Q, a, b, c)
\end{align*}

Next, we compute $\ell$ uniformly random coefficients $\texttt{input\_coef} \in \mathbb{F}_q^{|\texttt{input}|}$ and compute the sharing with \textit{Share}:
\begin{align}
  \sh{\texttt{input}}_i & = \text{Share}(i, \texttt{input}, \texttt{input\_coef}_1, \dots, \texttt{input\_coef}_\ell) \label{sdith-mpc-share} \\ \nonumber
  &= \texttt{input} + \sum_{j=1}^\ell i^j \cdot \texttt{input\_coef}_j
\end{align}

The MPC computation is then performed directly on the \texttt{input} and its $\ell$ coefficients from the SSSS:
\begin{align*}
  \texttt{input}            & \stackrel{\varphi^1, \varphi^2}{\longrightarrow} \texttt{broad}            \\
...
\begin{align}
  (\sh{\alpha}_i, \sh{\beta}_i, \sh{v}_i) = \text{Share}(i, \texttt{broad}, \texttt{broad\_coef}_1, \dots, \texttt{broad\_coef}_\ell)
\end{align}
...
\begin{protocol}[SD-in-the-Head Signing Protocol]
  ...
  \begin{itemize}[itemsep=0pt, topsep=0pt, parsep=0pt]
    ...
    \item a $(\ell + 1, N)$-threshold MPC protocol \texttt{MPC} (\autoref{def:sdith-mpc}, \autoref{sec:mpc})
  ...
  \begin{enumerate}[itemsep=0pt, topsep=0pt, parsep=0pt]
    \item Compute Beaver triples $\mathcal{B} = a^{j}, b^{j}, c^{j}$ for all $j \in t$ and construct $\texttt{input} = (sk, \mathcal{B})$
    \item For each iteration $e \in [\tau]$
          \begin{enumerate}[itemsep=0pt, topsep=0pt, parsep=0pt]
            \item compute $\texttt{input\_coef}_j^e \leftarrow \texttt{XOF}$ for each $j \in \ell$ 
            \item compute $\sh{\texttt{input}}_i^e = \text{Share}(i, \texttt{input}, \texttt{input\_coef}_1^e, \dots, \texttt{input\_coef}_\ell^e)$
            \item compute commitments for $i \in [N]$
            \begin{align*}
              \texttt{com}_i^{e} := \texttt{Hash}_0(\texttt{salt}, e, i, \sh{\texttt{input}}_i^e)
            \end{align*}
  ...
  \begin{enumerate}[itemsep=0pt, topsep=0pt, parsep=0pt]
    \item Compute $h_1 = \texttt{Hash}_1(pk, \texttt{salt}, \texttt{root}^{1}, \ldots, \texttt{root}^{\tau})$.
  ...
  \begin{enumerate}[itemsep=0pt, topsep=0pt, parsep=0pt]
    \item Compute plain broadcast $\alpha^{j}, \beta^{j} \leftarrow \texttt{MPC}(pk, \sh{a}^{j, e}, \sh{b}^{j, e}, \sh{c}^{j, e}, r, \varepsilon)$ for $j \in [t]$
    \item For $i \in S$, compute:
          \begin{align*}
            \sh{\alpha^{e}}_i := \phi_{x, \epsilon^{e}}(\sh{sk^{e}}_i, \sh{\beta^{e}}_i),
          \end{align*}
  \end{enumerate}
