...
//! For example, we can easily generate an array of random values in [F_q](crate::subroutines::arithmetics::FieldArith) by
...
#[cfg(feature = "kat")]
fn kat_xof_final(xof: &mut Shake) {
    let mut tmp = [0u8; 0];
    xof.squeeze(&mut tmp);
}
