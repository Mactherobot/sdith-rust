...
impl MPC {
    ...
    pub(crate) fn expand_mpc_challenges(h1: Hash, n: usize) -> Vec<Challenge> {
        ...
        for _ in 0..n {
            challenges.push(Challenge::new(h1.clone()));
        }
    ...
    pub(crate) fn expand_view_challenge_hash(digest: &Hash) -> Array2D<u16> {
        ...
        let mut opened_views = Array2D::new(PARAM_L, PARAM_TAU); // Array for storing the opened views
...
mod mpc_tests {
    ...
    fn prepare() -> (Input, Broadcast, Challenge, HPrimeMatrix, Vec<u8>) {
        ...
        let mut prg = PRG::init(&mseed, Some(&vec![0; PARAM_SALT_SIZE]));
        ...
        let chal = Challenge::new(&vec![0u8; PARAM_DIGEST_SIZE]);
    ...
    fn test_expand_view_challenge_hash() {
        ...
        let mut h2 = vec![0u8; PARAM_DIGEST_SIZE];
        for _ in 0..1000 {
    ...
    fn test_compute_broadcast() {
        ...
        let hash1 = &vec![0u8; PARAM_DIGEST_SIZE];
